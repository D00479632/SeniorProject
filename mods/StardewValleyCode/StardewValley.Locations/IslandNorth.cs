...
namespace StardewValley.Locations
{
	public class IslandNorth : IslandLocation
	{
		...
		public override void DayUpdate(int dayOfMonth)
		{
			...
			digSiteUpdate();
			terrainFeatures.RemoveWhere((KeyValuePair<Vector2, TerrainFeature> pair) => pair.Value is HoeDirt hoeDirt && hoeDirt.crop != null && hoeDirt.crop.forageCrop.Value);
			...
			if (Utility.getNumObjectsOfIndexWithinRectangle(digSiteBounds, new string[9] { "(O)816", "(O)817", "(O)818", "(O)819", "(O)32", "(O)38", "(O)40", "(O)42", "(O)590" }, this) < 60)
			{
				for (int j = 0; j < numberOfAdditionsToTry; j++)
				{
					...
					else if (Game1.random.NextDouble() < 0.1)
					{
						...
						if (CanItemBePlacedHere(position2, itemIsPassable: false, CollisionMask.All, CollisionMask.None) && !IsTileOccupiedBy(position2) && !hasTileAt(xCoord, yCoord, "AlwaysFront") && !hasTileAt(xCoord, yCoord, "Front") && !isBehindBush(position2) && doesTileHaveProperty(xCoord, yCoord, "Diggable", "Back") != null && doesTileHaveProperty(xCoord, yCoord, "Diggable", "Back") == "T")
						{
