	...
	public class MineShaft : GameLocation
	{
		...
		private void setElevatorLit()
		{
			if (ElevatorLightSpot.X != -1 && ElevatorLightSpot.Y != -1)
			{
				...
				defaultInterpolatedStringHandler.AppendFormatted(mineLevel);
				defaultInterpolatedStringHandler.AppendLiteral("_Elevator");
				currentLightSources.Add(new LightSource(defaultInterpolatedStringHandler.ToStringAndClear(), 4, new Vector2(ElevatorLightSpot.X, ElevatorLightSpot.Y) * 64f, 2f, Color.Black, LightSource.LightContext.None, 0L, base.NameOrUniqueName));
				...
				for (int x = 0; x < buildingsLayer.LayerWidth; x++)
				{
					...
					if (tileIndex != -1)
					{
						...
						if (!lighting.Equals(Color.White))
						{
							...
							case 113:
							{
								NetStringDictionary<LightSource, NetRef<LightSource>> dictionary = sharedLights;
								...
								dictionary.AddLight(new LightSource(defaultInterpolatedStringHandler.ToStringAndClear(), 4, new Vector2(x, y) * 64f, 2.5f, new Color(0, 50, 100), LightSource.LightContext.None, 0L, base.NameOrUniqueName));
								switch (tileIndex)
								{
								case 66:
					...
					{
						NetStringDictionary<LightSource, NetRef<LightSource>> dictionary2 = sharedLights;
						...
						dictionary2.AddLight(new LightSource(defaultInterpolatedStringHandler.ToStringAndClear(), 4, new Vector2(x, y) * 64f, 2f, new Color(0, 220, 220), LightSource.LightContext.None, 0L, base.NameOrUniqueName));
					}
				}
			}
			...
			if (mineLevel > 1 && (mineLevel % 5 != 0 || mineLevel >= 121) && (mineRandom.NextBool() || isDinoArea))
			{
				...
				for (int k = 0; k < numBarrels; k++)
				{
					...
					while (isTileOnMap(p.X, p.Y))
					{
						...
						{
							if (Utility.GetDayOfPassiveFestival("DesertFestival") > 0 && getMineArea() == 121 && !hasAddedDesertFestivalStatue && hasTileAt((int)objectPos5.X, (int)objectPos5.Y - 1, "Buildings"))
							{
					...
					for (int l = 0; l < backLayer.LayerHeight; l++)
					{
						...
						if (isTileClearForMineObjects(j, l))
						{
							...
							else if (mineRandom.NextDouble() <= monsterChance && getDistanceFromStart(j, l) > 5f)
							{
								...
								if (!(monsterToAdd2 is GreenSlime slime))
								{
							...
							{
								if (getMineArea() == 40 && mineLevel % 40 < 30 && mineRandom.NextDouble() < 0.01 && hasTileAt(j, l - 1, "Buildings"))
								{
									terrainFeatures.Add(new Vector2(j, l), new Tree("8", 5));
								}
								else if (getMineArea() == 40 && mineLevel % 40 < 30 && mineRandom.NextDouble() < 0.1 && (hasTileAt(j, l - 1, "Buildings") || hasTileAt(j - 1, l, "Buildings") || hasTileAt(j, l + 1, "Buildings") || hasTileAt(j + 1, l, "Buildings") || terrainFeatures.ContainsKey(new Vector2(j - 1, l)) || terrainFeatures.ContainsKey(new Vector2(j + 1, l)) || terrainFeatures.ContainsKey(new Vector2(j, l - 1)) || terrainFeatures.ContainsKey(new Vector2(j, l + 1))))
								{
									terrainFeatures.Add(new Vector2(j, l), new Grass((mineLevel >= 50) ? 6 : 5, (mineLevel >= 50) ? 1 : mineRandom.Next(1, 5)));
								}
								else if (getMineArea() == 80 && !isDarkArea() && mineRandom.NextDouble() < 0.1 && (hasTileAt(j, l - 1, "Buildings") || hasTileAt(j - 1, l, "Buildings") || hasTileAt(j, l + 1, "Buildings") || hasTileAt(j + 1, l, "Buildings") || terrainFeatures.ContainsKey(new Vector2(j - 1, l)) || terrainFeatures.ContainsKey(new Vector2(j + 1, l)) || terrainFeatures.ContainsKey(new Vector2(j, l - 1)) || terrainFeatures.ContainsKey(new Vector2(j, l + 1))))
								{
		...
		public void loadLevel(int level)
		{
			...
			if (forceLayout.HasValue)
			{
				...
				if (!mapContent.DoesAssetExist<Map>(assetName))
				{
					...
					defaultInterpolatedStringHandler.AppendFormatted(mapNumberToLoad);
		...
		{
			TemporaryAnimatedSpriteList temporaryAnimatedSpriteList = temporarySprites;
			TemporaryAnimatedSprite obj = new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(536, 1945, 8, 8), new Vector2(8.75f, 5.8f) * 64f + new Vector2(32f, -32f), flipped: false, 0f, Color.White)
			{
				interval = 50f,
			...
			obj.lightId = defaultInterpolatedStringHandler.ToStringAndClear();
			obj.id = 888;
			obj.lightRadius = 2f;
			obj.scale = 4f;
			obj.yPeriodic = true;
			obj.lightcolor = new Color(100, 0, 0);
			obj.yPeriodicLoopTime = 1000f;
			obj.yPeriodicRange = 4f;
			obj.layerDepth = 0.04544f;
			temporaryAnimatedSpriteList.Add(obj);
			TemporaryAnimatedSpriteList temporaryAnimatedSpriteList2 = temporarySprites;
			TemporaryAnimatedSprite obj2 = new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(536, 1945, 8, 8), new Vector2(10.75f, 5.8f) * 64f + new Vector2(32f, -32f), flipped: false, 0f, Color.White)
			{
				interval = 50f,
				totalNumberOfLoops = 99999,
				animationLength = 4
			...
			obj2.lightId = defaultInterpolatedStringHandler.ToStringAndClear();
			obj2.id = 889;
			obj2.lightRadius = 2f;
			obj2.scale = 4f;
			obj2.lightcolor = new Color(100, 0, 0);
			obj2.yPeriodic = true;
			obj2.yPeriodicLoopTime = 1100f;
			obj2.yPeriodicRange = 4f;
			obj2.layerDepth = 0.04544f;
			temporaryAnimatedSpriteList2.Add(obj2);
					...
					for (int x = 0; x < map.RequireLayer("Buildings").LayerWidth; x++)
					{
						...
						if (water_property != null)
						{
							...
							if (getMineArea() == 80 && Game1.random.NextDouble() < 0.1)
							{
								...
								defaultInterpolatedStringHandler.AppendFormatted(y);
								defaultInterpolatedStringHandler.AppendLiteral("_Lava");
								dictionary.AddLight(new LightSource(defaultInterpolatedStringHandler.ToStringAndClear(), 4, new Vector2(x, y) * 64f, 2f, new Color(0, 220, 220), LightSource.LightContext.None, 0L, base.NameOrUniqueName));
							}
						}
					}
				}
		...
		public static string GetLevelName(int level, int? forceLayout = null)
		{
			...
			defaultInterpolatedStringHandler.AppendFormatted(level);
			defaultInterpolatedStringHandler.AppendLiteral(":");
			defaultInterpolatedStringHandler.AppendFormatted(forceLayout);
			return defaultInterpolatedStringHandler.ToStringAndClear();
		}
			...
			foreach (MineShaft mine in activeMines)
			{
				if (mine.Name == name)
				{
					if (forceLayout.HasValue && mine.loadedMapNumber != forceLayout)
					{
						...
						defaultInterpolatedStringHandler.AppendFormatted(mine.loadedMapNumber);
						defaultInterpolatedStringHandler.AppendLiteral(".");
						log.Warn(defaultInterpolatedStringHandler.ToStringAndClear());
					}
					return mine;
