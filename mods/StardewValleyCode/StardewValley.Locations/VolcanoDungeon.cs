	...
	public class VolcanoDungeon : IslandLocation
	{
		...
		public virtual void GenerateLevel(bool use_level_level_as_layout = false)
		{
			...
			ApplyToColor(Color.Blue, delegate(int x, int y)
			{
				...
				if (generationRandom.NextDouble() < 0.1)
				{
					...
					defaultInterpolatedStringHandler.AppendFormatted(level.Value);
					...
					dictionary.AddLight(new LightSource(defaultInterpolatedStringHandler.ToStringAndClear(), 4, new Vector2(x, y) * 64f, 2f, new Color(0, 50, 50), LightSource.LightContext.None, 0L, base.NameOrUniqueName));
				}
			});
		...
		public virtual void GenerateEntities()
		{
			...
			if (level.Value != 5)
			{
				for (int x2 = 0; x2 < map.Layers[0].LayerWidth; x2++)
				{
					for (int y2 = 0; y2 < map.Layers[0].LayerHeight; y2++)
					{
						...
						{
							if (getTileIndexAt((int)objectPos.X, (int)objectPos.Y, "Back", "dungeon") == 25)
							{
								if (!isMushroomLevel())
								{
							...
							{
								characters.Add(new Bat(objectPos * 64f, (level.Value > 5 && generationRandom.NextBool()) ? (-556) : (-555)));
							}
						}
						else
						{
			...
			foreach (Point p in spiker_spawn_points)
			{
				...
				{
					int direction = 1;
					switch (getTileIndexAt(p, "Back", "dungeon"))
					{
					case 537:
					case 538:
						direction = 2;
