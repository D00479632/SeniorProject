		...
		protected override void resetLocalState()
		{
			...
			if (IsOwnedByCurrentPlayer && !Game1.player.activeDialogueEvents.ContainsKey("pennyRedecorating"))
			{
				...
				if (whichQuilt != -1 && !Game1.player.mailReceived.Contains("pennyRefurbished"))
				{
					...
					foreach (Furniture item in furniture)
					{
						if (item is BedFurniture bed_furniture && bed_furniture.bedType == BedFurniture.BedType.Double)
						{
							string bedId = null;
			...
			if (previousUpgradeLevel >= 0)
			{
				if (previousUpgradeLevel < 2 && upgradeLevel >= 2)
				{
					for (int x2 = 0; x2 < map.Layers[0].LayerWidth; x2++)
					{
						...
						{
							if (doesTileHaveProperty(x2, y2, "DefaultChildBedPosition", "Back") != null)
							{
								string bedId2 = BedFurniture.CHILD_BED_INDEX;
								furniture.Add(new BedFurniture(bedId2, new Vector2(x2, y2)));
								break;
							}
						}
					}
				...
				if (previousUpgradeLevel == 0)
				{
					foreach (Furniture item in furniture)
					{
						if (item is BedFurniture bed2 && bed2.bedType == BedFurniture.BedType.Single)
						{
							bed_furniture = bed2;
							break;
						}
				...
				else
				{
					...
					{
						if (item2 is BedFurniture bed && bed.bedType == BedFurniture.BedType.Double)
						{
							bed_furniture = bed;
							break;
						}
