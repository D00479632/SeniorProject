	...
	public class IslandLocation : GameLocation
	{
		...
		protected void addMoonlightJellies(int numTries, Random r, Microsoft.Xna.Framework.Rectangle exclusionRect)
		{
			for (int i = 0; i < numTries; i++)
			{
				...
				if (!tooClose)
				{
					TemporaryAnimatedSpriteList temporaryAnimatedSpriteList = underwaterSprites;
					TemporaryAnimatedSprite obj = new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle((r.NextDouble() < 0.2) ? 304 : 256, (r.NextDouble() < 0.01) ? 32 : 16, 16, 16), 250f, 3, 9999, new Vector2(tile.X, tile.Y) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White * 0.66f, 4f, 0f, 0f, 0f)
					{
