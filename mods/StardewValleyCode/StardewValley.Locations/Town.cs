		...
		public void showImprovedPamHouse()
		{
			...
			foreach (Point point in buildingsLayerBounds.GetPoints())
			{
				...
				if (buildingsLayer.Tiles[x2, y2] != null)
				{
					...
					if (buildingsLayer.GetTileIndexAt(x2, y2, "Town") == 1568)
					{
			...
			foreach (Point point2 in alwaysFrontLayerBounds.GetPoints())
			{
				...
				{
					alwaysFrontLayer.Tiles[x, y] = new StaticTile(alwaysFrontLayer, map.RequireTileSheet("Town"), BlendMode.Alpha, 1336 + (x - alwaysFrontLayerBounds.X) + (y - alwaysFrontLayerBounds.Y) * 32);
				}
			}
			if (!Game1.eventUp)
			{
		...
		public override void MakeMapModifications(bool force = false)
		{
			...
			if (Utility.doesMasterPlayerHaveMailReceivedButNotMailForTomorrow("ccMovieTheater"))
			{
				if (Utility.doesMasterPlayerHaveMailReceivedButNotMailForTomorrow("ccMovieTheaterJoja"))
				{
					...
					ApplyMapOverride("Town-TheaterCC" + (isHalloweenAlternate ? "-Halloween2" : ""), rect2, rect2);
				}
				else
				{
		...
		protected override void resetLocalState()
		{
			...
			if (num)
			{
				...
				Action<int, int> obj = delegate(int x, int y)
				{
					...
					currentLightSources.Add(new LightSource(defaultInterpolatedStringHandler.ToStringAndClear(), 9, new Vector2(x, y) * 64f + new Vector2(128f, 160f), 1f, Color.Black * 0.66f, LightSource.LightContext.None, 0L, base.NameOrUniqueName));
				};
				obj(32, 83);
				obj(42, 96);
				obj(50, 88);
