			...
			else
			{
				...
				for (int i = 0; i < getTilesWide(); i++)
				{
					if (environment != null)
					{
						...
						defaultInterpolatedStringHandler.AppendLiteral("_tile");
						defaultInterpolatedStringHandler.AppendFormatted(i);
						environment.removeLightSource(defaultInterpolatedStringHandler.ToStringAndClear());
					}
				}
				base.lightSource = null;
			}
		...
		public override bool canBePlacedHere(GameLocation l, Vector2 tile, CollisionMask collisionMask = CollisionMask.All, bool showError = false)
		{
			...
			for (int x = 0; x < tilesWide; x++)
			{
				for (int y = 0; y < tilesHigh; y++)
				{
					...
					if (l.terrainFeatures.GetValueOrDefault(curTile) is HoeDirt dirt && dirt.crop != null)
					{
						return false;
					}
				}
