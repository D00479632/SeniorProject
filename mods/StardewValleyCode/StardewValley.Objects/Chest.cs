	...
	public class Chest : Object
	{
		...
		public bool TryMoveToSafePosition(int? preferDirection = null)
		{
			...
			bool TryMoveRecursively(Vector2 tile_position, int depth, Vector2? prioritize_direction)
			{
				...
				foreach (Vector2 offset2 in offsets)
				{
					...
					{
						if (!location.objects.ContainsKey(new_position2) && location.objects.Remove(TileLocation))
						{
							kickStartTile.Value = TileLocation;
		...
		public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
		{
			...
			if (playerChest.Value && (base.QualifiedItemId == "(BC)130" || base.QualifiedItemId == "(BC)232" || base.QualifiedItemId.Equals("(BC)BigChest") || base.QualifiedItemId.Equals("(BC)BigStoneChest")))
			{
				...
				spriteBatch.Draw(texture3, Game1.GlobalToLocal(Game1.viewport, new Vector2(draw_x * 64f, draw_y * 64f + 20f)), new Microsoft.Xna.Framework.Rectangle(0, spriteIndex / 8 * 32 + 53, 16, 11), Color.White * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, base_sort_order + 2E-05f);
			...
			if (playerChest.Value)
			{
				...
				Texture2D texture2 = itemData3.GetTexture();
				spriteBatch.Draw(texture2, Game1.GlobalToLocal(Game1.viewport, new Vector2(draw_x * 64f + (float)((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), (draw_y - 1f) * 64f)), itemData3.GetSourceRect(), tint.Value * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, base_sort_order);
				spriteBatch.Draw(texture2, Game1.GlobalToLocal(Game1.viewport, new Vector2(draw_x * 64f + (float)((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), (draw_y - 1f) * 64f)), itemData3.GetSourceRect(0, currentLidFrame), tint.Value * alpha * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, base_sort_order + 1E-05f);
				return;
			}
			if (giftbox.Value)
			{
