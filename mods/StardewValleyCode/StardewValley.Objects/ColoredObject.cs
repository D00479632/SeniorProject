	...
	public class ColoredObject : Object
	{
		...
		public override void drawWhenHeld(SpriteBatch spriteBatch, Vector2 objectPosition, Farmer f)
		{
			...
			else if (!ColorSameIndexAsParentSheetIndex)
			{
				...
				spriteBatch.Draw(itemData2.GetTexture(), objectPosition, itemData2.GetSourceRect(1, base.ParentSheetIndex), color.Value, 0f, Vector2.Zero, 4f, SpriteEffects.None, Math.Max(0f, (float)(f.StandingPixel.Y + 4) / 10000f));
			}
			else
			{
		...
		public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
		{
			...
			else if (!Game1.eventUp || Location.IsFarm)
			{
				...
				{
					Rectangle coloredSourceRect = itemData.GetSourceRect(1, base.ParentSheetIndex);
					spriteBatch.Draw(texture, Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32, y * 64 + 32)), itemData.GetSourceRect(0, base.ParentSheetIndex), Color.White, 0f, new Vector2(8f, 8f), (scale.Y > 1f) ? getScale().Y : 4f, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (float)bounds.Bottom / 10000f);
					spriteBatch.Draw(texture, Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32 + ((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), y * 64 + 32 + ((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0))), coloredSourceRect, color.Value, 0f, new Vector2(8f, 8f), (scale.Y > 1f) ? getScale().Y : 4f, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (float)bounds.Bottom / 10000f);
				}
				else
				{
					spriteBatch.Draw(texture, Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32 + ((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), y * 64 + 32 + ((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0))), itemData.GetSourceRect(0, base.ParentSheetIndex), color.Value, 0f, new Vector2(8f, 8f), (scale.Y > 1f) ? getScale().Y : 4f, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (float)bounds.Bottom / 10000f);
				}
			}
		}
