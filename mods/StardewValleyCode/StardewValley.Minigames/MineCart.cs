...
namespace StardewValley.Minigames
{
	public class MineCart : IMinigame
	{
		...
		public bool tick(GameTime time)
		{
			...
			{
				if (_entities[m] != null && _entities[m].IsActive())
				{
					_entities[m].Update(delta_time);
				}
			}
			...
			if (gameState == GameStates.Ingame)
			{
				...
				if (checkpointPositions.Count > 0)
				{
					...
					while (k < checkpointPositions.Count && player.position.X >= (float)(checkpointPositions[k] * tileSize))
					{
						...
						{
							if (entity3 is CheckpointIndicator indicator2 && (int)(indicator2.position.X / (float)tileSize) == checkpointPositions[k])
							{
								indicator2.Activate();
								break;
							}
				...
				if (_goalIndicator != null && distanceToTravel != -1 && player.position.X >= _goalIndicator.position.X && distanceToTravel != -1 && player.position.Y <= _goalIndicator.position.Y * (float)tileSize + 4f && !reachedFinish && fadeDelta < 0f)
				{
					...
					foreach (Entity entity2 in _entities)
					{
						if (entity2 is GoalIndicator indicator)
						{
							indicator.Activate();
						}
		...
		public void ShowMap()
		{
			...
			case 9:
			{
				for (int n = 0; n < 8; n++)
				{
					AddEntity(new MineDebris(new Rectangle(368, 784, 16, 16), new Vector2(274 + Game1.random.Next(-19, 20), 46 + Game1.random.Next(6, 14)), Game1.random.Next(-4, 5), -16f, 0f, 0.05f, 2f, 1f, 3, 0.33f, 0.35f, holdLastFrame: true, 1f + (float)n * 0.1f)).SetStartSound("dirtyHit");
				}
				break;
			}
			case 6:
			{
				for (int m = 0; m < 52; m++)
				{
					AddEntity(new CosmeticFallingBoulder(Game1.random.Next(72, 195), new Color(100, 66, 49), 96 + Game1.random.Next(-10, 11), 0.65f + (float)m * 0.05f)).position = new Vector2(5 + Game1.random.Next(360), -16f);
				}
				break;
			}
			}
