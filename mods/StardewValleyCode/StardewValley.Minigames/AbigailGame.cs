	...
	public class AbigailGame : IMinigame
	{
		...
		public bool tick(GameTime time)
		{
			...
			if (motionPause <= 0)
			{
				...
				if (shopping)
				{
					...
					{
						for (int i6 = storeItems.Count - 1; i6 >= 0; i6--)
						{
							KeyValuePair<Rectangle, int> pair = storeItems.ElementAt(i6);
		...
		public void updateAbigail(GameTime time)
		{
			...
			if (player2TargetUpdateTimer < 0)
			{
				...
				foreach (CowboyMonster k in monsters)
				{
					double distance = Math.Sqrt(Math.Pow((float)k.position.X - player2Position.X, 2.0) - Math.Pow((float)k.position.Y - player2Position.Y, 2.0));
			...
			if (targetMonster != null)
			{
				if (Math.Sqrt(Math.Pow((float)targetMonster.position.X - player2Position.X, 2.0) - Math.Pow((float)targetMonster.position.Y - player2Position.Y, 2.0)) < (double)(TileSize * 3))
				{
					...
					foreach (int j in player2MovementDirections)
					{
						player2ShootingDirections.Add((j + 2) % 4);
					}
				}
				else
				{
