...
namespace StardewValley.Buildings
{
	...
	public class Building : INetObject<NetFields>, IHaveModData
	{
		...
		public virtual bool CanBePainted()
		{
			...
			if ((isCabin || HasIndoorsName("Farmhouse")) && GetIndoors() is FarmHouse house && house.upgradeLevel < 2)
			{
				return false;
			}
		...
		public virtual bool hasCarpenterPermissions()
		{
			...
			if (GetIndoors() is FarmHouse farmHouse && farmHouse.IsOwnedByCurrentPlayer)
			{
				return true;
			}
			return false;
		...
		public virtual void InitializeIndoor(BuildingData data, bool forConstruction, bool forUpgrade)
		{
			...
			if (forUpgrade && data.IndoorItemMoves != null)
			{
				...
				{
					for (int x2 = 0; x2 < move.Size.X; x2++)
					{
						for (int y2 = 0; y2 < move.Size.Y; y2++)
						{
							interior.moveContents(move.Source.X + x2, move.Source.Y + y2, move.Destination.X + x2, move.Destination.Y + y2, move.UnlessItemId);
						}
					}
				}
			}
			...
			foreach (IndoorItemAdd item in data.IndoorItems)
			{
				...
				if (item.ClearTile)
				{
					...
					{
						int y = 0;
						for (int height = newFurniture.getTilesHigh(); y < height; y++)
						{
							int x = 0;
							for (int width = newFurniture.getTilesWide(); x < width; x++)
							{
								interior.cleanUpTileForMapOverride(new Point((int)tileVector.X + x, (int)tileVector.Y + y), item.ItemId);
							}
						}
					}
					else
		...
		public virtual void performActionOnConstruction(GameLocation location, Farmer who)
		{
			...
			{
				location.localSound("axchop", buildingCenter);
				for (int x2 = tileX.Value; x2 < tileX.Value + tilesWide.Value; x2++)
				{
					for (int y2 = tileY.Value; y2 < tileY.Value + tilesHigh.Value; y2++)
					{
						for (int j = 0; j < 5; j++)
						{
				...
				for (int k = 0; k < 8; k++)
				{
					DelayedAction.playSoundAfterDelay("dirtyHit", 250 + k * 150, location, buildingCenter, -1, local: true);
				}
			}
			else
			{
				...
				Microsoft.Xna.Framework.Rectangle sourceRectForMenu = getSourceRectForMenu().GetValueOrDefault(mainSourceRect);
				int y = 0;
				for (int bottomEdge = mainSourceRect.Height / 16 * 2; y <= bottomEdge; y++)
				{
					...
					for (int rightEdge = sourceRectForMenu.Width / 16 * 2; x < rightEdge; x++)
					{
						...
						location.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(666, 1851, 8, 8), 40f, 4, 2, new Vector2(tileX.Value, tileY.Value) * 64f + new Vector2(x * 64 / 2, y * 64 / 2 - mainSourceRect.Height * 4 + tilesHigh.Value * 64) + new Vector2(Game1.random.Next(-32, 32), Game1.random.Next(-32, 32)), flicker: false, flipped: false)
						{
							layerDepth = (float)((tileY.Value + tilesHigh.Value) * 64) / 10000f + (float)x / 10000f + 0.0001f,
							pingPong = true,
							delayBeforeAnimationStart = (mainSourceRect.Height / 16 * 2 - y) * 100,
							scale = 4f,
							alphaFade = 0.01f,
			...
			if (GetIndoors() is Cabin cabin && !cabin.HasOwner)
			{
		...
		public virtual void Update(GameTime time)
		{
			...
			if (!hasChimney.HasValue)
			{
				...
				{
					hasChimney = true;
					string[] split2 = ArgUtility.SplitBySpace(chimneyString);
			...
			if (interior is FarmHouse farmhouse)
			{
				...
				if (lastHouseUpgradeLevel != upgradeLevel)
				{
					...
					{
						hasChimney = true;
						string[] split = ArgUtility.SplitBySpace(chimneyString2);
						chimneyPosition.X = int.Parse(split[0]);
						chimneyPosition.Y = int.Parse(split[1]);
					}
				}
			}
			if (!hasChimney.GetValueOrDefault() || interior == null)
			{
				return;
			}
		...
		public virtual void drawInConstruction(SpriteBatch b)
		{
			...
			if (magical.Value || daysOfConstructionLeft.Value <= 0)
			{
				...
				if (magical.Value)
				{
					...
					{
						b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(tileX.Value * 64 + j * 16, (float)(tileY.Value * 64 - mainSourceRect.Height * 4 + tilesHigh.Value * 64) + (float)(mainSourceRect.Height * 4) * (1f - drawPercentageReal))) + new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2) - ((j % 2 == 0) ? 32 : 8)), new Microsoft.Xna.Framework.Rectangle(536 + (newConstructionTimer.Value + j * 4) % 56 / 8 * 8, 1945, 8, 8), (j % 2 == 1) ? (Color.Pink * alpha) : (Color.LightPink * alpha), 0f, new Vector2(0f, 0f), 4f + (float)Game1.random.Next(100) / 100f, SpriteEffects.None, (float)((tileY.Value + tilesHigh.Value) * 64) / 10000f + 0.0001f);
						if (j % 2 == 0)
						{
							b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(tileX.Value * 64 + j * 16, (float)(tileY.Value * 64 - mainSourceRect.Height * 4 + tilesHigh.Value * 64) + (float)(mainSourceRect.Height * 4) * (1f - drawPercentageReal))) + new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2) + ((j % 2 == 0) ? 32 : 8)), new Microsoft.Xna.Framework.Rectangle(536 + (newConstructionTimer.Value + j * 4) % 56 / 8 * 8, 1945, 8, 8), Color.White * alpha, 0f, new Vector2(0f, 0f), 4f + (float)Game1.random.Next(100) / 100f, SpriteEffects.None, (float)((tileY.Value + tilesHigh.Value) * 64) / 10000f + 0.0001f);
						}
					}
					return;
				}
				...
				{
					b.Draw(Game1.animations, Game1.GlobalToLocal(Game1.viewport, new Vector2(tileX.Value * 64 - 16 + i * 16, (float)(tileY.Value * 64 - mainSourceRect.Height * 4 + tilesHigh.Value * 64) + (float)(mainSourceRect.Height * 4) * (1f - drawPercentageReal))) + new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2) - ((i % 2 == 0) ? 32 : 8)), new Microsoft.Xna.Framework.Rectangle((newConstructionTimer.Value + i * 20) % 304 / 38 * 64, 768, 64, 64), Color.White * alpha * ((float)newConstructionTimer.Value / 500f), 0f, new Vector2(0f, 0f), 1f, SpriteEffects.None, (float)((tileY.Value + tilesHigh.Value) * 64) / 10000f + 0.0001f);
					if (i % 2 == 0)
					{
						b.Draw(Game1.animations, Game1.GlobalToLocal(Game1.viewport, new Vector2(tileX.Value * 64 - 16 + i * 16, (float)(tileY.Value * 64 - mainSourceRect.Height * 4 + tilesHigh.Value * 64) + (float)(mainSourceRect.Height * 4) * (1f - drawPercentageReal))) + new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2) - ((i % 2 == 0) ? 32 : 8)), new Microsoft.Xna.Framework.Rectangle((newConstructionTimer.Value + i * 20) % 400 / 50 * 64, 2944, 64, 64), Color.White * alpha * ((float)newConstructionTimer.Value / 500f), 0f, new Vector2(0f, 0f), 1f, SpriteEffects.None, (float)((tileY.Value + tilesHigh.Value) * 64) / 10000f + 0.0001f);
					}
				}
				return;
			}
