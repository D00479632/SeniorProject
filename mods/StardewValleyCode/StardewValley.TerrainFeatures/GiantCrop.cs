...
namespace StardewValley.TerrainFeatures
{
	public class GiantCrop : ResourceClump
	{
			...
			if (t.hasEnchantmentOfType<ShavingEnchantment>() && r.NextBool((float)power / 5f) && data?.HarvestItems != null)
			{
				foreach (GiantCropHarvestItemData drop2 in data.HarvestItems)
				{
					...
					if (item2 != null)
					{
						...
						Debris d = new Debris(item2, new Vector2((tileLocation.X + (float)(width.Value / 2)) * 64f, (tileLocation.Y + (float)(height.Value / 2)) * 64f), Game1.player.getStandingPosition());
			...
			if (health.Value <= 0f)
			{
				...
				if (data?.HarvestItems != null)
				{
					foreach (GiantCropHarvestItemData drop in data.HarvestItems)
					{
						...
						{
							Game1.createMultipleItemDebris(item, new Vector2((int)tileLocation.X + width.Value / 2, (int)tileLocation.Y + width.Value / 2) * 64f, -2, location);
		...
		public Item TryGetDrop(GiantCropHarvestItemData drop, Random r, Farmer targetFarmer, bool isShaving, float healthDeducted)
		{
			...
			GiantCropHarvestItemData data = drop;
			GameLocation location = Location;
			...
			Item item = ItemQueryResolver.TryResolveRandomItem(data, new ItemQueryContext(location, targetFarmer, r, defaultInterpolatedStringHandler.ToStringAndClear()), avoidRepeat: false, null, null, null, delegate(string query, string error)
			{
