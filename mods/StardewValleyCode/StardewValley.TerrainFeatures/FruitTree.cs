...
namespace StardewValley.TerrainFeatures
{
	public class FruitTree : TerrainFeature
	{
		...
		private Item TryCreateFruit(FruitTreeFruitData drop)
		{
			...
			defaultInterpolatedStringHandler.AppendLiteral("'");
			Item item = ItemQueryResolver.TryResolveRandomItem(data, new ItemQueryContext(location, null, null, defaultInterpolatedStringHandler.ToStringAndClear()), avoidRepeat: false, null, null, null, delegate(string query, string error)
			{
				IGameLogger log = Game1.log;
				...
				defaultInterpolatedStringHandler2.AppendFormatted(treeId.Value);
		...
		public override bool performToolAction(Tool t, int explosion, Vector2 tileLocation)
		{
			...
			if (growthStage.Value >= 4)
			{
				...
				else
				{
					...
					damage2 = t.upgradeLevel.Value switch
					{
						0 => 1f, 
						1 => 1.25f, 
						2 => 1.67f, 
		...
		public override void draw(SpriteBatch spriteBatch)
		{
			...
			if (growthStage.Value < 4)
			{
				...
				Rectangle sourceRect2 = growthStage.Value switch
				{
				...
				spriteBatch.Draw(this.texture, Game1.GlobalToLocal(Game1.viewport, new Vector2(tileLocation.X * 64f + 32f + positionOffset.X, tileLocation.Y * 64f - (float)sourceRect2.Height + 128f + positionOffset.Y)), sourceRect2, Color.White, shakeRotation, new Vector2(24f, 80f), 4f, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (float)boundingBox.Bottom / 10000f - tileLocation.X / 1000000f);
			}
			else
			{
				if (!stump.Value || falling.Value)
