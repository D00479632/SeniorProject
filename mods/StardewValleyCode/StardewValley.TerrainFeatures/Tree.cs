...
namespace StardewValley.TerrainFeatures
{
	public class Tree : TerrainFeature
	{
		...
		public Item TryGetDrop(WildTreeItemData drop, Random r, Farmer targetFarmer, string fieldName, Func<string, string> formatItemId = null, bool? isStump = null)
		{
			...
			defaultInterpolatedStringHandler.AppendLiteral("'");
			return ItemQueryResolver.TryResolveRandomItem(data, new ItemQueryContext(location, targetFarmer, r, defaultInterpolatedStringHandler.ToStringAndClear()), avoidRepeat: false, null, formatItemId, null, delegate(string query, string error)
			{
				...
				defaultInterpolatedStringHandler2.AppendFormatted(treeType.Value);
		...
		public override void dayUpdate()
		{
			...
			if (growthStage.Value >= 5 && !stump.Value && environment is Farm && Game1.random.NextBool(data?.SeedSpreadChance ?? 0.15f))
			{
				...
				{
					environment.terrainFeatures.Add(location, new Tree(treeType.Value, 0));
				}
			}
			if (isTemporaryGreenRainTree.Value && environment.IsGreenhouse && (localSeason.GetValueOrDefault() == Season.Winter || localSeason.GetValueOrDefault() == Season.Fall))
			{
				hasSeed.Value = false;
			}
			else
			...
			if (!environment.IsGreenhouse && (localSeason.GetValueOrDefault() == Season.Winter || stump.Value))
			{
				hasMoss.Value = false;
				return;
			}
			bool? flag = data?.GrowsMoss;
			if (flag.HasValue && flag.GetValueOrDefault() && growthStage.Value >= 14 && !stump.Value && Game1.random.NextBool(mossChance))
			{
				hasMoss.Value = true;
			}
		}
