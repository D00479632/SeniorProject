...
namespace StardewValley.Internal
{
	...
	public readonly struct ForEachItemContext
	{
		...
		{
			if (!(pathValue is GameLocation location2))
			{
				...
				if (!(pathValue is Building building))
				{
					...
					else
					{
						...
						defaultInterpolatedStringHandler.AppendFormatted(parentObj.TileLocation.X);
						defaultInterpolatedStringHandler.AppendLiteral(", ");
						defaultInterpolatedStringHandler.AppendFormatted(parentObj.TileLocation.Y);
				...
				if (path.Count == 0)
				{
					...
					if (location != null)
					{
						AddDisplayPath(path, location);
					}
				}
				defaultInterpolatedStringHandler = new DefaultInterpolatedStringHandler(6, 3);
				defaultInterpolatedStringHandler.AppendFormatted(building.buildingType.Value);
				defaultInterpolatedStringHandler.AppendLiteral(" at ");
				defaultInterpolatedStringHandler.AppendFormatted(building.tileX.Value);
				defaultInterpolatedStringHandler.AppendLiteral(", ");
				defaultInterpolatedStringHandler.AppendFormatted(building.tileY.Value);
				path.Add(defaultInterpolatedStringHandler.ToStringAndClear());
			}
			else
			{
				if (path.Count == 0 && location2.ParentBuilding != null)
				{
					AddDisplayPath(path, location2.ParentBuilding);
				}
				path.Add(location2.NameOrUniqueName);
			}
		}
	}
}
