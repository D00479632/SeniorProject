	...
	public static class ChatCommands
	{
		...
		public static class DefaultHandlers
		{
			...
			public static void Emote(string[] command, ChatBox chat)
			{
				...
				if (command.Length > 1)
				{
					...
					{
						if (emoteType == Farmer.EMOTES[j].emoteString)
						{
							validEmote = true;
							break;
						}
				...
				for (int i = 0; i < Farmer.EMOTES.Length; i++)
				{
					...
					{
						emoteList += Farmer.EMOTES[i].emoteString;
			...
			public static void Help(string[] command, ChatBox chat)
			{
				...
				{
					if (Handlers.TryGetValue(searchCommandName, out var handler2))
					{
						string description = handler2.HelpDescription?.Invoke(handler2.Name);
						if (description != null)
						{
				...
				foreach (ChatCommand handler in Handlers.Values)
				{
					if (handler.IsVisible() && handler.HelpDescription?.Invoke(handler.Name) != null)
					{
						if (handler.IsMultiplayerOnly)
						{
							multiplayerCommandNames.Add(handler.Name);
						}
						else
						{
							commandNames.Add(handler.Name);
						}
					}
				}
			...
			public static void MoveBuildingPermission(string[] command, ChatBox chat)
			{
				...
				defaultInterpolatedStringHandler.AppendFormatted(Game1.player.team.farmhandsCanMoveBuildings.Value);
			...
			public static void Unban(string[] command, ChatBox chat)
			{
				...
				if (command.Length > 1)
				{
					...
					if (userId != null)
					{
						...
						chat.addInfoMessage(Game1.content.LoadString("Strings\\UI:ChatCommands_Unban_Done", userDisplay2));
				...
				foreach (KeyValuePair<string, string> bannedUser in Game1.bannedUsers)
				{
					...
					if (bannedUser.Value != null)
					{
						...
						defaultInterpolatedStringHandler.AppendFormatted(bannedUser.Value);
						defaultInterpolatedStringHandler.AppendLiteral(" (");
						defaultInterpolatedStringHandler.AppendFormatted(bannedUser.Key);
						defaultInterpolatedStringHandler.AppendLiteral(")");
						userDisplay = defaultInterpolatedStringHandler.ToStringAndClear();
					}
					chat.addInfoMessage(userDisplay);
				}
			}
			...
			public static void UnlinkPlayer(string[] command, ChatBox chat)
			{
				...
				if (farmer != null)
				{
					...
					defaultInterpolatedStringHandler.AppendFormatted(farmer.uniqueMultiplayerID);
					defaultInterpolatedStringHandler.AppendLiteral(" ('");
		...
		public static bool AllowCheats
		{
			get
			{
				...
				{
					return (Game1.player?.team?.allowChatCheats.Value).GetValueOrDefault();
				}
				return true;
			}
		}
