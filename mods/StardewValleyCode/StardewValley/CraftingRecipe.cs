...
namespace StardewValley
{
	public class CraftingRecipe
	{
		...
		public static bool TryParseLevelRequirement(string id, string rawData, bool isCooking, out int skillNumber, out int minLevel, bool logErrors = true)
		{
			...
			int argIndex = 1;
			string text = ArgUtility.Get(parts, 0)?.ToLower();
			if (text != null)
			{
				string skillId;
				string error2;
				...
				case 4:
					...
					IL_0169:
					if (ArgUtility.TryGet(parts, argIndex, out skillId, out error2, allowBlank: true, "string skillId") && ArgUtility.TryGetInt(parts, argIndex + 1, out minLevel, out error2, "minLevel"))
					{
			...
			void LogFormatWarning(string error)
			{
				...
				defaultInterpolatedStringHandler.AppendFormatted(isCooking ? "Cooking" : "Crafting");
				...
				defaultInterpolatedStringHandler.AppendFormatted(conditions);
		...
		public static void ConsumeAdditionalIngredients(List<KeyValuePair<string, int>> additionalRecipeItems, List<IInventory> additionalMaterials)
		{
			...
			{
				string itemId = additionalRecipeItems[k].Key;
				int requiredCount = additionalRecipeItems[k].Value;
				bool foundInBackpack = false;
				for (int j = Game1.player.Items.Count - 1; j >= 0; j--)
				{
					Item item2 = Game1.player.Items[j];
					if (ItemMatchesForCrafting(item2, itemId))
					{
						int toRemove2 = Math.Min(requiredCount, item2.Stack);
						requiredCount -= toRemove2;
						Game1.player.Items[j] = item2.ConsumeStack(toRemove2);
						if (requiredCount <= 0)
						{
							foundInBackpack = true;
							break;
