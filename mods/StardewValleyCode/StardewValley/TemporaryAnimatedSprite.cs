						...
						if (fireworkType >= 0)
						{
							...
							if (Game1.IsMasterGame)
							{
								...
								for (int j = 0; j < array.Length; j++)
								{
									...
									TemporaryAnimatedSprite obj = new TemporaryAnimatedSprite("LooseSprites\\Cursors_1_6", new Rectangle(0, 0, 1, 1), 1800f * mult, 1, 0, position, flicker: false, flipped: false, -1f, 0f, Color.Transparent, 1f, 0f, 0f, 0f)
									{
										motion = point,
									...
									defaultInterpolatedStringHandler.AppendFormatted(point.X);
									defaultInterpolatedStringHandler.AppendLiteral("_");
									defaultInterpolatedStringHandler.AppendFormatted(point.Y);
									obj.lightId = defaultInterpolatedStringHandler.ToStringAndClear();
									obj.lightRadius = 1.3f;
									obj.drawAboveAlwaysFront = true;
									obj.lightFade = 2;
									fireworkSprites.Add(obj);
								}
								array = points;
								for (int j = 0; j < array.Length; j++)
								{
								...
								if (id == 30)
								{
									for (int i = 0; i < 8; i++)
									{
										...
										TemporaryAnimatedSprite obj2 = new TemporaryAnimatedSprite("LooseSprites\\Cursors_1_6", new Rectangle(304, 397, 11, 11), 75f * mult, 12, 5, position, flicker: false, flipped: false, -1f, 0f, Utility.getBlendedColor(Color.White, Utility.getRandomRainbowColor()), 4f, 0f, 0f, 0f)
										{
											motion = mot * 1.1f,
											alpha = 1f,
											alphaFade = 0.01f,
										...
										obj2.lightRadius = 0.33f;
										obj2.lightFade = 3;
										fireworkSprites.Add(obj2);
									}
								}
