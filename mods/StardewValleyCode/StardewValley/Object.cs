	...
	public class Object : Item
	{
			...
			if (qualifiedItemId != null)
			{
				...
				case 7:
					...
					case '2':
						...
						break;
					case '5':
						if (!(qualifiedItemId == "(BC)165") || !(location is AnimalHouse animalHouse) || !(heldObject.Value is Chest chest))
						{
							break;
						}
						...
						break;
					case '6':
						if (!(qualifiedItemId == "(BC)156") || MinutesUntilReady > 0 || heldObject.Value == null)
						{
							break;
						}
					...
					case '4':
						...
						{
							if (!(qualifiedItemId == "(BC)164") || !(location is Town))
							{
								break;
							}
				...
				case 6:
					...
					break;
				case 15:
					if (qualifiedItemId == "(BC)MushroomLog" && Game1.IsRainingHere(location))
					{
						minutesUntilReady.Value -= Utility.CalculateMinutesUntilMorning(Game1.timeOfDay);
					}
					break;
		...
		public virtual bool OutputMachine(MachineData machine, MachineOutputRule outputRule, Item inputItem, Farmer who, GameLocation location, bool probe, bool heldObjectOnly = false)
		{
			...
			if (!heldObjectOnly)
			{
				...
				minutesUntilReady = (MinutesUntilReady = (int)Utility.ApplyQuantityModifiers(minutesUntilReady, machine.ReadyTimeModifiers, machine.ReadyTimeModifierMode, location, who, heldObject.Value, inputItem));
				if (MinutesUntilReady == 0)
				{
					...
					if (qualifiedItemId != null)
					{
						...
						case 6:
						{
							...
							case '6':
								...
								sprinkleColor = ((base.QualifiedItemId == "(O)681") ? Color.SlateBlue : ((base.QualifiedItemId == "(O)688") ? Color.LimeGreen : ((base.QualifiedItemId == "(O)689") ? Color.OrangeRed : ((base.QualifiedItemId == "(O)261") ? new Color(255, 200, 0) : Color.LightBlue))));
								...
								sprite = new TemporaryAnimatedSprite(0, 9999f, 1, 999, Game1.player.Position + new Vector2(0f, -96f), flicker: false, flipped: false, verticalFlipped: false, 0f)
								{
		...
		public override string getCategoryName()
		{
			...
			{
				return furniture.placementRestriction switch
				{
		...
		protected bool CheckForActionOnMachine(Farmer who, bool justCheckingForActivity = false)
		{
			...
			if (readyForHarvest.Value)
			{
				...
				if (lastOutputRuleId.Value != null)
				{
					...
					{
						heldObject.Value = null;
						OutputMachine(machineData2, outputRule, lastInputItem.Value, who, location, probe: false, heldObjectOnly: true);
						if (heldObject.Value != null)
						{
				...
				if (who.IsLocalPlayer)
				{
					...
					checkForReload = true;
					MachineDataUtility.UpdateStats(machineData2?.StatsToIncrementWhenHarvested, outputObj, outputObj.Stack);
				}
				heldObject.Value = null;
				readyForHarvest.Value = false;
				showNextIndex.Value = false;
				ResetParentSheetIndex();
				if (MachineDataUtility.TryGetMachineOutputRule(this, machineData2, MachineOutputTrigger.OutputCollected, outputObj.getOne(), who, location, out var outputCollectedRule, out var _, out var _, out var _))
				{
					OutputMachine(machineData2, outputCollectedRule, lastInputItem.Value, who, location, probe: false);
				}
				if (IsTapper() && location.terrainFeatures.TryGetValue(tileLocation.Value, out var terrainFeature) && terrainFeature is Tree tree)
				{
		...
		public virtual void farmerAdjacentAction(Farmer who, bool diagonal = false)
		{
			...
			else if (!diagonal)
			{
				...
				if (Location.objects.TryGetValue(v, out var tileObj) && tileObj.IsTextSign())
				{
		...
		public virtual void drawAsProp(SpriteBatch b)
		{
			...
			else
			{
				...
				ParsedItemData itemData = ItemRegistry.GetDataOrErrorItem(base.QualifiedItemId);
				b.Draw(itemData.GetTexture(), Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32, y * 64 + 32)), itemData.GetSourceRect(), Color.White, 0f, new Vector2(8f, 8f), (scale.Y > 1f) ? getScale().Y : 4f, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (float)bounds.Bottom / 10000f);
			}
		}
		...
		public override bool canBePlacedHere(GameLocation l, Vector2 tile, CollisionMask collisionMask = CollisionMask.All, bool showError = false)
		{
			...
			if (isWildTreeSeed(base.ItemId))
			{
				...
				if (!canPlaceWildTreeSeed(l, tile, out var deniedMessage3))
				{
			...
			case -74:
			{
				...
				if (IsTeaSapling())
				{
					...
					if (!l.CheckItemPlantRules(base.QualifiedItemId, isFreeGardenPot, l.isOutdoors.Value || l.IsGreenhouse, out var deniedMessage))
					{
		...
		{
			if (preservedId == "-1" && preserveType.GetValueOrDefault() == PreserveType.Honey)
			{
				preservedId = null;
			}
			return preservedId;
		...
		public virtual bool placementAction(GameLocation location, int x, int y, Farmer who = null)
		{
			...
			if (!bigCraftable.Value && !(this is Furniture))
			{
				...
				if (IsWildTreeSapling())
				{
					if (!canPlaceWildTreeSeed(location, placementTile, out var deniedMessage2))
					{
				...
				if (qualifiedItemId != null)
				{
					...
					case 6:
					{
						...
						case '9':
							...
							IL_1190:
							fireworkType = base.ParentSheetIndex - 893;
			...
			else
			{
				if (IsTapper())
				{
					if (location.terrainFeatures.TryGetValue(placementTile, out var terrainFeature6) && terrainFeature6 is Tree tree3 && tree3.growthStage.Value >= 5 && !tree3.stump.Value && !location.objects.ContainsKey(placementTile) && (!tree3.isTemporaryGreenRainTree.Value || Game1.season != Season.Summer))
					{
			...
			if (base.Category == -19 && location.terrainFeatures.TryGetValue(placementTile, out var terrainFeature3) && terrainFeature3 is HoeDirt dirt3 && dirt3.crop != null && (base.QualifiedItemId == "(O)369" || base.QualifiedItemId == "(O)368") && dirt3.crop.currentPhase.Value != 0)
			{
			...
			if (isSapling())
			{
				...
				{
					if (!(terrainFeature2 is HoeDirt dirt2) || dirt2.crop != null)
					{
						return false;
					}
					location.terrainFeatures.Remove(placementTile);
			...
			if (base.Category == -74 || base.Category == -19)
			{
				if (location.terrainFeatures.TryGetValue(placementTile, out var terrainFeature))
				{
								...
								if (base.Category == -74)
								{
									foreach (Object o in location.Objects.Values)
									{
										...
										chest.GetMutex().RequestLock(delegate
										{
											...
											{
												Item item = items[0];
												if (item.Category == -19 && dirt.plant(item.ItemId, who, isFertilizer: true))
												{
		...
		public virtual string GenerateLightSourceId(Vector2 position)
		{
			...
			defaultInterpolatedStringHandler.AppendLiteral("_");
			defaultInterpolatedStringHandler.AppendFormatted(position.X);
			defaultInterpolatedStringHandler.AppendLiteral("_");
			defaultInterpolatedStringHandler.AppendFormatted(position.Y);
			return defaultInterpolatedStringHandler.ToStringAndClear();
		}
		...
		protected virtual float getPriceAfterMultipliers(float startPrice, long specificPlayerID = -1L)
		{
			...
			foreach (Farmer player in Game1.getAllFarmers())
			{
				...
				if (player.professions.Contains(6) && (base.Category == -4 || (preserve != null && preserve.Value.HasValue && preserve.Value.GetValueOrDefault() == PreserveType.SmokedFish)))
				{
