	...
	public class GameLocation : INetObject<NetFields>, IEquatable<GameLocation>, IAnimalLocation, IHaveModData
	{
		...
		public virtual void HandleGrassGrowth(int dayOfMonth)
		{
			if (dayOfMonth == 1)
			{
				...
				{
					terrainFeatures.RemoveWhere((KeyValuePair<Vector2, TerrainFeature> pair) => pair.Value is HoeDirt hoeDirt && hoeDirt.crop == null && Game1.random.NextDouble() < 0.8);
				}
		...
		public virtual Warp getWarpFromDoor(Point door, Character character = null)
		{
			...
			if (propertyName != null)
			{
				...
				if (c != 'B')
				{
					if (c != 'M')
					{
						if (c == 'e' && propertyName == "LockedDoorWarp")
						{
							goto IL_0229;
						}
					}
				...
				else if (propertyName == "WarpBoatTunnel")
				{
					if (!(character is NPC))
					{
						...
						if (tmp != null)
						{
							...
							if (!tmp.TileIndexProperties.ContainsKey("Shadow") && !tmp.TileIndexProperties.ContainsKey("Passable") && !tmp.Properties.ContainsKey("Passable") && (!projectile || (!tmp.TileIndexProperties.ContainsKey("ProjectilePassable") && !tmp.Properties.ContainsKey("ProjectilePassable"))))
							{
								...
								if (!tmp.TileIndexProperties.ContainsKey("NPCPassable") && !tmp.Properties.ContainsKey("NPCPassable"))
								{
									...
									if (!flag2.HasValue || !flag2.GetValueOrDefault() || !tmp.Properties.ContainsKey("Action"))
									{
										goto IL_01c9;
									}
								}
			...
			try
			{
				...
				else
				{
					...
					case 11:
					{
						...
						for (int k = 0; k < 40; k++)
						{
							Game1.multiplayer.broadcastSprites(this, new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(666, 1851, 8, 8), 25f, 4, 2, new Vector2(3f, 19f) * 64f + new Vector2(-8 + k % 4 * 16, -(k / 4) * 64 / 4), flicker: false, flipped: false)
							{
								layerDepth = 0.1152f + (float)k / 10000f,
								color = new Color(100 + k * 4, k * 5, 120 + k * 4),
								pingPong = true,
								delayBeforeAnimationStart = k * 10,
								scale = 4f,
								alphaFade = 0.01f
							});
							Game1.multiplayer.broadcastSprites(this, new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(666, 1851, 8, 8), 25f, 4, 2, new Vector2(3f, 17f) * 64f + new Vector2(-8 + k % 4 * 16, k / 4 * 64 / 4), flicker: false, flipped: false)
							{
								layerDepth = 0.1152f + (float)k / 10000f,
								color = new Color(232 - k * 4, 192 - k * 6, 255 - k * 4),
								pingPong = true,
								delayBeforeAnimationStart = 320 + k * 10,
								scale = 4f,
								alphaFade = 0.01f
							});
							Game1.multiplayer.broadcastSprites(this, new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(666, 1851, 8, 8), 25f, 4, 2, new Vector2(3f, 19f) * 64f + new Vector2(-8 + k % 4 * 16, -(k / 4) * 64 / 4), flicker: false, flipped: false)
							{
								layerDepth = 0.1152f + (float)k / 10000f,
								color = new Color(100 + k * 4, k * 6, 120 + k * 4),
								pingPong = true,
								delayBeforeAnimationStart = 640 + k * 10,
								scale = 4f,
								alphaFade = 0.01f
							});
						}
		...
		public virtual void UpdateWhenCurrentLocation(GameTime time)
		{
			...
			{
				furniture[j].updateWhenCurrentLocation(time);
			}
			...
			for (int i = _activeTerrainFeatures.Count - 1; i >= 0; i--)
			{
				TerrainFeature feature2 = _activeTerrainFeatures[i];
			...
			if (Game1.gameMode == 3 && this == Game1.currentLocation)
			{
				...
				foreach (Farmer farmer in farmers)
				{
					...
					for (int k = 0; k < directionsTileVectorsWithDiagonals.Length; k++)
					{
						Vector2 offset2 = directionsTileVectorsWithDiagonals[k];
		...
		public virtual void monsterDrop(Monster monster, int x, int y, Farmer who)
		{
			...
			if (who.stats.Get("Book_Void") != 0 && Game1.random.NextDouble() < 0.03 && debrisToAdd != null && monster != null)
			{
				foreach (Debris d in debrisToAdd)
				{
					...
					{
						Item tmp = ItemRegistry.Create(d.itemId.Value);
		...
		public bool damageMonster(Microsoft.Xna.Framework.Rectangle areaOfEffect, int minDamage, int maxDamage, bool isBomb, float knockBackModifier, int addedPrecision, float critChance, float critMultiplier, bool triggerMonsterInvincibleTimer, Farmer who, bool isProjectile = false)
		{
			...
			for (int i = characters.Count - 1; i >= 0; i--)
			{
				if (i < characters.Count && characters[i] is Monster monster && monster.IsMonster && monster.Health > 0 && monster.TakesDamageFromHitbox(areaOfEffect))
				{
		...
		public virtual void DayUpdate(int dayOfMonth)
		{
			...
			{
				KeyValuePair<Vector2, TerrainFeature> pair2 = array2[j];
			...
			if (map != null && (isOutdoors.Value || map.Properties.ContainsKey("ForceSpawnForageables")) && !map.Properties.ContainsKey("skipWeedGrowth"))
			{
				if (Game1.dayOfMonth % 7 == 0 && !(this is Farm))
				{
					...
					{
						KeyValuePair<Vector2, Object> pair4 = array4[j];
					...
					for (int x = 0; x < map.Layers[0].LayerWidth; x++)
					{
						for (int y = 0; y < map.Layers[0].LayerHeight; y++)
						{
							...
							if (GetFurnitureAt(tile) == null && !terrainFeatures.ContainsKey(tile) && !objects.ContainsKey(tile) && getBuildingAt(tile) == null)
							{
								if (isFruitTree)
								{
									terrainFeatures.Add(tile, new FruitTree(treeId, growthStageOnRegrow.GetValueOrDefault(2)));
								}
								else
								{
									terrainFeatures.Add(tile, new Tree(treeId, growthStageOnRegrow.GetValueOrDefault(2)));
								}
							}
						}
					}
		...
		public void setFireplace(bool on, int tileLocationX, int tileLocationY, bool playSound = true, int xOffset = 0, int yOffset = 0)
		{
			...
			defaultInterpolatedStringHandler.AppendFormatted(tileLocationX);
			defaultInterpolatedStringHandler.AppendLiteral("_");
			defaultInterpolatedStringHandler.AppendFormatted(tileLocationY);
		...
		public void tryAddPrismaticButterfly()
		{
			...
			{
				if (critter is Butterfly b && b.isPrismatic)
				{
					return;
				}
			}
		...
		public void performGreenRainUpdate()
		{
			...
			{
				if (!ArgUtility.TryGetVector2(trees, j, out var position, out var error, integerOnly: false, "Vector2 position") || !ArgUtility.TryGetInt(trees, j + 2, out var treeType, out error, "int treeType"))
				{
		...
		public void setUpLocationSpecificFlair()
		{
			...
			case 9:
				...
				case 'L':
					if (value == "LeahHouse")
					{
						...
						{
							string toSay5 = Game1.random.Next(3) switch
							{
			...
			case 12:
				...
				case 'S':
					...
					{
						NPC p7 = Game1.getCharacterFromName("Robin");
						if (p7 != null && p7.TilePoint.Y == 18)
						{
							...
							if (Game1.random.NextDouble() < 0.001)
							{
								toSay10 = "Strings\\SpeechBubbles:ScienceHouse_Robin_RareGreeting";
							}
							p7.showTextAboveHead(Game1.content.LoadString(toSay10, Game1.player.Name));
						}
					}
				...
				case 'E':
					if (value == "ElliottHouse")
					{
						...
						{
							string toSay9 = Game1.random.Next(3) switch
							{
							...
							e.showTextAboveHead(Game1.content.LoadString(toSay9, Game1.player.Name));
						}
					}
					break;
				}
			...
			case 8:
				...
				case 'H':
				{
					...
					NPC p3 = Game1.getCharacterFromName("Maru");
					if (p3 != null && p3.currentLocation == this && !p3.isDivorcedFrom(Game1.player))
					{
						...
						if (Game1.player.spouse == "Maru")
						{
							...
							p3.showTextAboveHead(Game1.content.LoadString(toSay3), SpriteText.color_Red);
						}
						else
						{
				...
				case 'J':
					...
					if (Game1.random.NextBool())
					{
						...
						{
							string toSay4 = "Strings\\SpeechBubbles:JojaMart_Morris_Greeting";
				...
				case 'S':
					...
					{
						NPC p2 = Game1.getCharacterFromName("Pierre");
						if (p2 != null && p2.TilePoint.Y == 17 && p2.currentLocation == this)
						{
							...
							string dialogue = Game1.content.LoadString("Strings\\SpeechBubbles:SeedShop_Pierre_" + toSay2);
							p2.showTextAboveHead(string.Format(dialogue, Game1.player.Name));
						}
					}
			...
			case 10:
				...
				case 'A':
					...
					if (Game1.random.NextBool())
					{
						NPC p6 = Game1.getCharacterFromName("Marnie");
						if (p6 != null && p6.TilePoint.Y == 14)
						{
							...
							if (Game1.random.NextDouble() < 0.001)
							{
								toSay8 = "Strings\\SpeechBubbles:AnimalShop_Marnie_RareGreeting";
							}
							p6.showTextAboveHead(Game1.content.LoadString(toSay8, Game1.player.Name, Game1.player.farmName));
						}
					}
				...
				case 'S':
					...
					if (Game1.random.NextBool())
					{
						NPC p5 = Game1.getCharacterFromName("Sandy");
				...
				case 'M':
					if (value == "ManorHouse")
					{
						...
						{
							string toSay6 = ((Game1.timeOfDay < 1200) ? "Morning" : ((Game1.timeOfDay < 1700) ? "Afternoon" : "Evening"));
							le.faceTowardFarmerForPeriod(3000, 15, faceAway: false, Game1.player);
							le.showTextAboveHead(Game1.content.LoadString("Strings\\SpeechBubbles:ManorHouse_Lewis_" + toSay6));
						}
					}
					break;
				case 'G':
			...
			case 14:
				...
				if (Game1.random.NextBool())
				{
					NPC p8 = Game1.getCharacterFromName("Marlon");
					if (p8 != null)
					{
						...
						p8.showTextAboveHead(Game1.content.LoadString(toSay12));
					}
				}
				break;
			case 16:
		...
		protected virtual void resetLocalState()
		{
			...
			if (!ignoreLights.Value)
			{
				...
				{
					if (!ArgUtility.TryGetPoint(lights, l, out var tile3, out var error3, "Point tile") || !ArgUtility.TryGetInt(lights, l + 2, out var textureIndex, out error3, "int textureIndex"))
					{
					...
					defaultInterpolatedStringHandler.AppendLiteral("_");
					defaultInterpolatedStringHandler.AppendFormatted(tile3.X);
					defaultInterpolatedStringHandler.AppendLiteral("_");
					defaultInterpolatedStringHandler.AppendFormatted(tile3.Y);
					currentLightSources.Add(new LightSource(defaultInterpolatedStringHandler.ToStringAndClear(), textureIndex, new Vector2(tile3.X * 64 + 32, tile3.Y * 64 + 32), 1f, LightSource.LightContext.MapLight, 0L, NameOrUniqueName));
				}
				if (!Game1.isTimeToTurnOffLighting(this) && !Game1.isRaining)
				{
					...
					{
						if (!ArgUtility.TryGetPoint(windowLights, k, out var tile4, out var error4, "Point tile") || !ArgUtility.TryGetInt(windowLights, k + 2, out var textureIndex2, out error4, "int textureIndex"))
						{
							LogMapPropertyError("WindowLight", windowLights, error4);
							continue;
						}
						Dictionary<string, LightSource> currentLightSources2 = Game1.currentLightSources;
						DefaultInterpolatedStringHandler defaultInterpolatedStringHandler = new DefaultInterpolatedStringHandler(9, 3);
						defaultInterpolatedStringHandler.AppendFormatted(lightIdPrefix);
						defaultInterpolatedStringHandler.AppendLiteral("_");
						defaultInterpolatedStringHandler.AppendFormatted(tile4.X);
						defaultInterpolatedStringHandler.AppendLiteral("_");
						defaultInterpolatedStringHandler.AppendFormatted(tile4.Y);
						defaultInterpolatedStringHandler.AppendLiteral("_Window");
						currentLightSources2.Add(new LightSource(defaultInterpolatedStringHandler.ToStringAndClear(), textureIndex2, new Vector2(tile4.X * 64 + 32, tile4.Y * 64 + 32), 1f, LightSource.LightContext.WindowLight, 0L, NameOrUniqueName));
					}
					foreach (Vector2 v in lightGlows)
					{
						Dictionary<string, LightSource> currentLightSources3 = Game1.currentLightSources;
						DefaultInterpolatedStringHandler defaultInterpolatedStringHandler = new DefaultInterpolatedStringHandler(7, 3);
						defaultInterpolatedStringHandler.AppendFormatted(lightIdPrefix);
						defaultInterpolatedStringHandler.AppendLiteral("_");
						defaultInterpolatedStringHandler.AppendFormatted(v.X);
						defaultInterpolatedStringHandler.AppendLiteral("_");
						defaultInterpolatedStringHandler.AppendFormatted(v.Y);
						defaultInterpolatedStringHandler.AppendLiteral("_Glow");
						currentLightSources3.Add(new LightSource(defaultInterpolatedStringHandler.ToStringAndClear(), 6, v, 1f, LightSource.LightContext.WindowLight, 0L, NameOrUniqueName));
					}
				}
			}
			if (isOutdoors.Value || treatAsOutdoors.Value)
			{
				...
				{
					if (!ArgUtility.TryGetVector2(sounds, j, out var tile, out var error, integerOnly: false, "Vector2 tile") || !ArgUtility.TryGetInt(sounds, j + 2, out var soundId, out error, "int soundId"))
					{
			...
			if (Game1.timeOfDay < night_tiles_time && (!IsRainingHere() || name.Equals("SandyHouse")))
			{
				...
				{
					if (!ArgUtility.TryGet(dayTiles, i, out var layerId, out var error2, allowBlank: true, "string layerId") || !ArgUtility.TryGetPoint(dayTiles, i + 1, out var position, out error2, "Point position") || !ArgUtility.TryGetInt(dayTiles, i + 3, out var tileIndex, out error2, "int tileIndex"))
					{
		...
		public virtual bool checkAction(Location tileLocation, xTile.Dimensions.Rectangle viewport, Farmer who)
		{
			...
			foreach (Furniture f in furniture)
			{
				if (f.boundingBox.Value.Contains((int)(tilePos.X * 64f), (int)(tilePos.Y * 64f)) && f.furniture_type.Value != 12)
				{
					if (didRightClick)
					{
				...
				if (f.furniture_type.Value == 6 && f.boundingBox.Value.Contains(vectOnWall))
				{
					paintingFound = f;
				}
			}
		...
		public virtual void ShowLockedDoorMessage(string[] action)
		{
			...
			else
			{
				...
				object obj = Game1.content.LoadStringReturnNullIfNotFound("Strings\\Locations:DoorUnlock_NotFriend_" + ownerName);
				if (obj == null)
				{
					...
					defaultInterpolatedStringHandler.AppendFormatted(ownerGender);
					obj = content.LoadStringReturnNullIfNotFound(defaultInterpolatedStringHandler.ToStringAndClear(), ownerDisplayName) ?? Game1.content.LoadString("Strings\\Locations:DoorUnlock_NotFriend_Female", ownerDisplayName);
				}
				lockedDoorMessage = (string)obj;
			}
		...
		public virtual bool performAction(string[] action, Farmer who, Location tileLocation)
		{
			...
			if (who.IsLocalPlayer)
			{
				...
				if (actionType != null)
				{
					...
					int mineLevel;
					Point playerTile;
					Microsoft.Xna.Framework.Rectangle ownerSearchArea;
					string id;
					Point tile2;
					...
					case 17:
					{
						...
						if ((uint)c <= 77u)
						{
							if (c != 'B')
							{
								if (c != 'M' || !(actionType == "MinecartTransport"))
								{
									break;
								}
						...
						{
							if (c != 'W' || !(actionType == "Warp_Sunroom_Door"))
							{
								break;
							}
					...
					case 18:
					{
						...
						if (c != 'C')
						{
							if (c != 'M')
							{
								if (c != 'r' || !(actionType == "GrandpaMasteryNote"))
								{
									break;
								}
					...
					case 20:
					{
						...
						if (c != 'F')
						{
							if (c != 'P' || !(actionType == "MasteryCave_Pedestal"))
							{
								break;
							}
					...
					case 19:
					{
						...
						if (c != 'C')
						{
							if (c != 'a')
							{
								if (c != 'i' || !(actionType == "MasteryCave_Fishing"))
								{
									break;
								}
					...
					case 15:
					{
						...
						if ((uint)c <= 100u)
						{
							if (c != 'S')
							{
								if (c != 'd' || !(actionType == "Arcade_Minecart"))
								{
									break;
								}
								if (who.hasSkullKey)
						...
						else if (c != 'i')
						{
							if (c != 't')
							{
								if (c != 'y' || !(actionType == "EmilyRoomObject"))
								{
									break;
								}
					...
					case 16:
					{
						...
						if ((uint)c <= 70u)
						{
							if (c != 'E')
							{
								if (c != 'F' || !(actionType == "FishingDerbySign"))
								{
									break;
								}
						...
						else if (c != 'Q')
						{
							if (c != 'T')
							{
								if (c != 'W' || !(actionType == "WarpWomensLocker"))
								{
									break;
								}
							...
							else
							{
								...
								if (Game1.MasterPlayer.hasOrWillReceiveMail("ccMovieTheater"))
								{
									...
									else if (Game1.player.lastSeenMovieWeek.Value >= Game1.Date.TotalWeeks)
									{
					...
					case 13:
					{
						...
						if ((uint)c <= 101u)
						{
							if (c != 'C')
							{
								if (c != 'c')
								{
									if (c != 'e' || !(actionType == "AdventureShop"))
									{
										break;
									}
									adventureShop();
						...
						else
						{
							if ((uint)c > 109u)
							{
								if (c != 's')
								{
									if (c != 't' || !(actionType == "NextMineLevel"))
									{
										break;
									}
									goto IL_38b3;
								}
								if (!(actionType == "MessageSpeech"))
								{
									break;
								}
								goto IL_327c;
							...
							if (c != 'm' || !(actionType == "SummitBoulder"))
							{
								break;
							}
						...
						if (c != 'B')
						{
							if (c == 'N')
							{
								...
								catch (Exception e)
								{
									...
									return LogError(defaultInterpolatedStringHandler.ToStringAndClear());
								}
							}
							if (c != 'T' || !(actionType == "Theater_PosterComingSoon"))
							{
								break;
							}
							if (Game1.MasterPlayer.hasOrWillReceiveMail("ccMovieTheater"))
							...
							if (who?.ActiveObject?.QualifiedItemId == "(O)103")
							{
								...
								tempSprite2.reachedStopCoordinate = delegate
								{
									...
									DelayedAction.functionAfterDelay(delegate
									{
										...
										for (int l = 0; l < 13; l++)
										{
											temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\terraria_cat", new Microsoft.Xna.Framework.Rectangle(134, (l == 12) ? 54 : 41, 21, 12), 9999f, 1, 1, tempSprite2.position, flicker: false, flipped: false, 0.97f - (float)l * 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f)
											{
												xPeriodic = true,
												xPeriodicLoopTime = 500 + Game1.random.Next(-50, 50),
												xPeriodicRange = 2f,
												motion = new Vector2(0f, -8f),
												delayBeforeAnimationStart = 220 + 80 * l
											});
										}
					...
					case 3:
						...
						if (!ArgUtility.TryGetRemainder(action, 1, out dialogue2, out error, ' ', "string dialogue"))
						{
							return LogError(error);
						}
						...
						if (!ArgUtility.TryGetRemainder(action, 1, out dialogue3, out error, ' ', "string dialogue"))
						{
							return LogError(error);
						}
						...
						goto IL_46ed;
						IL_327c:
						if (!ArgUtility.TryGet(action, 1, out translationKey, out error, allowBlank: true, "string translationKey"))
						{
							return LogError(error);
						}
						s = null;
						...
						if (!ArgUtility.TryGet(action, 1, out box_id, out error, allowBlank: true, "string box_id"))
						{
							return LogError(error);
						}
						minimum_capacity = 0;
						...
						if (!ArgUtility.TryGet(action, 1, out which2, out error, allowBlank: true, "string which"))
						{
							return LogError(error);
						}
						doStarpoint(which2);
						...
						if (!ArgUtility.TryGetInt(action, 1, out key, out error, "int key"))
						{
							return LogError(error);
						}
						...
						goto IL_46ed;
						IL_38b3:
						if (!ArgUtility.TryGetOptionalInt(action, 1, out mineLevel, out error, 1, "int mineLevel"))
						{
							return LogError(error);
						}
						...
						if (this is Farm && getBuildingAt(new Vector2(tileLocation.X, tileLocation.Y))?.GetIndoors() is FarmHouse farmhouse && !farmhouse.IsOwnedByCurrentPlayer)
						{
						...
						playerTile = who.TilePoint;
						ownerSearchArea = new Microsoft.Xna.Framework.Rectangle(playerTile.X - 1, playerTile.Y - 2, 2, 1);
						...
						if (!ArgUtility.TryGet(action, 1, out id, out error, allowBlank: true, "string id"))
						{
							return LogError(error);
						}
						...
						if (!ArgUtility.TryGetPoint(action, 1, out tile2, out error, "Point tile") || !ArgUtility.TryGet(action, 3, out locationName2, out error, allowBlank: true, "string locationName"))
						{
							return LogError(error);
						}
						playDoorSound = action.Length < 5;
						if (!who.IsMale)
						{
							if (who.IsLocalPlayer)
							{
						...
						if (!ArgUtility.TryGetPoint(action, 1, out tile3, out error, "Point tile") || !ArgUtility.TryGet(action, 3, out locationName3, out error, allowBlank: true, "string locationName") || !ArgUtility.TryGetInt(action, 4, out openTime, out error, "int openTime") || !ArgUtility.TryGetInt(action, 5, out closeTime, out error, "int closeTime") || !ArgUtility.TryGetOptional(action, 6, out npcName3, out error, null, allowBlank: true, "string npcName") || !ArgUtility.TryGetOptionalInt(action, 7, out minFriendship, out error, 0, "int minFriendship"))
						{
							return LogError(error);
						}
						who.faceGeneralDirection(new Vector2(tileLocation.X, tileLocation.Y) * 64f);
						lockedDoorWarp(tile3, locationName3, openTime, closeTime, npcName3, minFriendship);
						goto IL_46ed;
						IL_0b7b:
						...
						if (!ArgUtility.TryGet(action, 1, out npcName, out error, allowBlank: true, "string npcName") || !ArgUtility.TryGetRemainder(action, 2, out rawMessage, out error, ' ', "string rawMessage"))
						{
							return LogError(error);
						}
			...
			if (Game1.mailbox.Count > 0)
			{
				...
				if (mailTitle.StartsWith("passedOut"))
				{
					if (mailTitle.StartsWith("passedOut "))
					{
						...
						mail = Dialogue.applyGenderSwitchBlocks(str: mails[Utility.CreateDaySaveRandom(moneyTaken2).Next((Game1.player.getSpouse() != null && Game1.player.getSpouse().Name.Equals("Harvey")) ? 2 : 3) switch
						{
							0 => (Game1.MasterPlayer.hasCompletedCommunityCenter() && !Game1.MasterPlayer.mailReceived.Contains("JojaMember")) ? "passedOut4" : ("passedOut1_" + ((moneyTaken2 > 0) ? "Billed" : "NotBilled") + "_" + (Game1.player.IsMale ? "Male" : "Female")), 
							1 => "passedOut2", 
							_ => "passedOut3_" + ((moneyTaken2 > 0) ? "Billed" : "NotBilled"), 
					...
					else
					{
						...
						if (split.Length > 1)
						{
							...
							mail = Dialogue.applyGenderSwitchBlocks(Game1.player.Gender, mails[split[0]]);
		...
		public int getTotalCrops()
		{
			...
			{
				if (value is HoeDirt dirt && dirt.crop != null && !dirt.crop.dead.Value)
				{
					amount++;
				}
			}
		...
		public int getTotalUnwateredCrops()
		{
			...
			foreach (TerrainFeature value in terrainFeatures.Values)
			{
				if (value is HoeDirt dirt && dirt.crop != null && dirt.needsWatering() && !dirt.isWatered())
				{
					amount++;
				}
			}
		...
		public int getTotalOpenHoeDirt()
		{
			...
			{
				if (t is HoeDirt dirt && dirt.crop == null && !objects.ContainsKey(t.Tile))
				{
					amount++;
				}
			}
		...
		public virtual bool answerDialogueAction(string questionAndAnswer, string[] questionParams)
		{
			...
			if (questionAndAnswer != null)
			{
				if (questionAndAnswer != null)
				{
					...
					if (c != 'C')
					{
						if (c == 'G' && questionAndAnswer == "GoldClock_Yes")
						{
			...
			if (c != 'F')
			{
				if (c != 'M' || !(questionAndAnswer == "Mine_Yes"))
				{
					goto IL_393d;
				}
			...
			if ((uint)c <= 99u)
			{
				if (c != 'M')
				{
					if (c != 'c' || !(questionAndAnswer == "communityUpgrade_Yes"))
					{
						goto IL_393d;
					}
			...
			{
				if (c != 'p' || !(questionAndAnswer == "pagedResponse_cancel"))
				{
					goto IL_393d;
				}
			...
			goto IL_3976;
			IL_0129:
			c = questionAndAnswer[1];
			if ((uint)c <= 105u)
			{
				if (c != 'a')
				{
					if (c != 'i' || !(questionAndAnswer == "WizardShrine_Yes"))
					{
						goto IL_393d;
					}
			...
			else if (c != 'n')
			{
				if (c != 'o')
				{
					if (c != 'u' || !(questionAndAnswer == "BuyClubCoins_Yes"))
					{
						goto IL_393d;
					}
			...
			if (c != 'B')
			{
				...
				if (Game1.activeClickableMenu is CarpenterMenu menu3)
				{
					menu3.readOnly = true;
			...
			if (c != 'S')
			{
				if (c != 'p' || !(questionAndAnswer == "pagedResponse_nextPage"))
				{
					goto IL_393d;
				}
			...
			if (c != 'E')
			{
				if (c == 'M' && questionAndAnswer == "Marnie_Adopt")
				{
			...
			if ((uint)c <= 84u)
			{
				...
				if (c != 'S')
				{
					if (c != 'T' || !(questionAndAnswer == "ExitToTitle_Yes"))
					{
						goto IL_393d;
					}
			...
			{
				if (c != 'o' || !(questionAndAnswer == "buyJojaCola_Yes"))
				{
					goto IL_393d;
				}
			...
			if (c != 'H')
			{
				...
				else
				{
					...
					Utility.TryOpenShopMenu("Carpenter", null, playOpenSound: true);
					if (Game1.activeClickableMenu is ShopMenu menu5)
					{
						menu5.readOnly = true;
			...
			if (c != 'D')
			{
				if (c != 'M')
				{
					if (c != 'S' || !(questionAndAnswer == "Shaft_Jump"))
					{
						goto IL_393d;
					}
			...
			if ((uint)c <= 69u)
			{
				if (c != 'B')
				{
					if (c != 'E' || !(questionAndAnswer == "ExitMine_Go"))
					{
						goto IL_393d;
					}
					Game1.enterMine(Game1.CurrentMineLevel - 1);
			...
			else if (c != 'M')
			{
				if (c != 'm')
				{
					if (c != 'u' || !(questionAndAnswer == "upgrade_Yes"))
					{
						goto IL_393d;
					}
					houseUpgradeAccept();
			...
			if (c != 'C')
			{
				if (c != 'a')
				{
					if (c != 'c' || !(questionAndAnswer == "carpenter_Construct"))
					{
						goto IL_393d;
					}
			...
			if ((uint)c <= 108u)
			{
				if (c != 'a')
				{
					if (c != 'l' || !(questionAndAnswer == "ClearHouse_Yes"))
					{
						goto IL_393d;
					}
			...
			if (c != 'o')
			{
				if (c != 'u')
				{
					if (c == 'x' && questionAndAnswer == "ExitMine_Leave")
					{
						goto IL_31d7;
					}
				}
			...
			else if (questionAndAnswer == "Bookseller_Buy")
			{
				Utility.TryOpenShopMenu("Bookseller", null, playOpenSound: true);
				goto IL_3976;
			}
			...
			if ((uint)c <= 109u)
			{
				if (c != 'a')
				{
					...
					else
					{
						...
						RemoveProfession(24);
						Game1.drawObjectDialogue(Game1.content.LoadString("Strings\\Locations:Sewer_DogStatueFinished"));
						int num4 = Farmer.checkForLevelGain(0, Game1.player.experiencePoints[4]);
						if (num4 >= 5)
						{
			...
			goto IL_3976;
			IL_018f:
			c = questionAndAnswer[5];
			if ((uint)c <= 82u)
			{
				if (c != 'F')
				{
					if (c != 'R' || !(questionAndAnswer == "pagedResponse_previousPage"))
					{
						goto IL_393d;
					}
					_ShowPagedResponses(_PagedResponsePage - 1);
			...
			{
				if (c != 'n' || !(questionAndAnswer == "carpenter_CommunityUpgrade"))
				{
					goto IL_393d;
				}
		...
		public void explode(Vector2 tileLocation, int radius, Farmer who, bool damageFarmers = true, int damage_amount = -1, bool destroyObjects = true)
		{
			...
			TemporaryAnimatedSprite temporaryAnimatedSprite = new TemporaryAnimatedSprite(23, 9999f, 6, 1, new Vector2(currentTile.X * 64f, currentTile.Y * 64f), flicker: false, Game1.random.NextBool());
			...
			defaultInterpolatedStringHandler.AppendLiteral("_");
			defaultInterpolatedStringHandler.AppendFormatted(tileLocation.X);
			defaultInterpolatedStringHandler.AppendLiteral("_");
			defaultInterpolatedStringHandler.AppendFormatted(tileLocation.Y);
			defaultInterpolatedStringHandler.AppendLiteral("_");
			...
			temporaryAnimatedSprite.lightRadius = radius;
		...
		private string HandleTreasureTileProperty(int xLocation, int yLocation, bool detectOnly)
		{
			...
			int yLocation2 = yLocation;
			string property = doesTileHaveProperty(xLocation2, yLocation2, "Treasure", "Back");
			if (property == null)
			{
				return null;
			}
			...
			if (type != null)
			{
				...
				if (type == "Arch")
				{
					...
					{
						Game1.createObjectDebris(itemId, xLocation2, yLocation2);
					}
					else
					{
						LogError(property, error);
			...
			if (c != 'C')
			{
				if (c == 'O' && type == "Object")
				{
					if (ArgUtility.TryGet(fields, 1, out var itemId4, out error, allowBlank: true, "string itemId"))
					{
						Game1.createObjectDebris(itemId4, xLocation2, yLocation2);
						if (itemId4 == "78" || itemId4 == "(O)79")
						{
			...
			goto IL_03c5;
			IL_03e0:
			map.RequireLayer("Back").Tiles[xLocation2, yLocation2].Properties["Treasure"] = null;
			return type;
			IL_03c5:
			type = null;
			...
			{
				Item item = ItemRegistry.Create(itemId3);
				Game1.createItemDebris(item, new Vector2(xLocation2, yLocation2), -1, this);
			...
			if (ArgUtility.TryGetInt(fields, 1, out var itemId2, out error, "int itemId"))
			{
				Game1.createDebris(debris, xLocation2, yLocation2, itemId2);
			}
			else
			{
				LogError(property, error);
			...
			Game1.createObjectDebris("(O)330", xLocation2, yLocation2);
			goto IL_03e0;
			IL_0136:
			if (type == "Iron")
			{
			...
			Game1.createObjectDebris("(O)78", xLocation2, yLocation2);
			goto IL_03e0;
			void LogError(string value, string errorPhrase)
			{
				LogTilePropertyError("Treasure", "Back", xLocation2, yLocation2, value, errorPhrase);
			}
		}
		...
		public virtual void shiftContents(int dx, int dy, Func<Vector2, object, bool> where = null)
		{
			...
			foreach (KeyValuePair<Vector2, Object> v4 in list)
			{
				...
				{
					removeLightSource(v4.Value.lightSource?.Id);
		...
		public void OnStoneDestroyed(string stoneId, int x, int y, Farmer who)
		{
			...
			if (who?.currentLocation is MineShaft mine && mine.mineLevel > 120 && !mine.isSideBranch())
			{
				...
				if (possibleForage.Any())
				{
					...
					for (int j = 0; j < numberToSpawn; j++)
					{
						for (int attempt = 0; attempt < 11; attempt++)
						{
							...
							if (objects.ContainsKey(location2) || IsNoSpawnTile(location2) || doesTileHaveProperty(xCoord2, yCoord2, "Spawnable", "Back") == null || doesEitherTileOrTileIndexPropertyEqual(xCoord2, yCoord2, "Spawnable", "Back", "F") || !CanItemBePlacedHere(location2) || hasTileAt(xCoord2, yCoord2, "AlwaysFront") || hasTileAt(xCoord2, yCoord2, "AlwaysFront2") || hasTileAt(xCoord2, yCoord2, "AlwaysFront3") || hasTileAt(xCoord2, yCoord2, "Front") || isBehindBush(location2) || (!r.NextBool(0.1) && isBehindTree(location2)))
							{
								continue;
							}
		...
		public virtual void checkForEvents()
		{
			if (Game1.killScreen && !Game1.eventUp)
			{
				...
				else
				{
					try
					{
						if (TryGetLocationEvents(out var assetName, out var events2) && events2.TryGetValue("PlayerKilled", out var eventScript))
						{
		...
		protected void drawDebris(SpriteBatch b)
		{
			...
			foreach (Debris d in debris)
			{
				...
				if (d.item != null)
				{
					Vector2 position5 = d.Chunks[0].GetVisualPosition();
					if (d.item is Object obj && obj.bigCraftable.Value)
					{
						obj.drawInMenu(b, Utility.snapDrawPosition(Game1.GlobalToLocal(Game1.viewport, position5 + new Vector2(32f, 32f))), 1.6f, 1f, ((float)(d.chunkFinalYLevel + 64 + 8) + position5.X / 10000f) / 10000f, StackDrawType.Hide, Color.White, drawShadow: true);
					}
					else
					{
						d.item.drawInMenu(b, Utility.snapDrawPosition(Game1.GlobalToLocal(Game1.viewport, position5 + new Vector2(32f, 32f))), 0.8f + (float)d.itemQuality * 0.1f, 1f, ((float)(d.chunkFinalYLevel + 64 + 8) + position5.X / 10000f) / 10000f, StackDrawType.Hide, Color.White, drawShadow: true);
					}
					continue;
				}
				...
				case Debris.DebrisType.LETTERS:
				{
					...
					Game1.drawWithBorder(d.debrisMessage.Value, Color.Black, d.nonSpriteChunkColor.Value, Utility.snapDrawPosition(Game1.GlobalToLocal(Game1.viewport, position)), chunk.rotation, chunk.scale, (position.Y + 64f) / 10000f);
					continue;
				}
				case Debris.DebrisType.NUMBERS:
				{
					Chunk chunk3 = d.Chunks[0];
					Vector2 position3 = chunk3.GetVisualPosition();
					NumberSprite.draw(d.chunkType.Value, b, Game1.GlobalToLocal(Game1.viewport, Utility.snapDrawPosition(new Vector2(position3.X, (float)d.chunkFinalYLevel - ((float)d.chunkFinalYLevel - position3.Y)))), d.nonSpriteChunkColor.Value, chunk3.scale * 0.75f, 0.98f + 0.0001f * (float)counter, chunk3.alpha, -1 * (int)((float)d.chunkFinalYLevel - position3.Y) / 2);
					continue;
				}
				case Debris.DebrisType.SPRITECHUNKS:
				{
					for (int i = 0; i < d.Chunks.Count; i++)
					{
						...
						Vector2 position2 = chunk2.GetVisualPosition();
						b.Draw(d.spriteChunkSheet, Utility.snapDrawPosition(Game1.GlobalToLocal(Game1.viewport, position2)), new Microsoft.Xna.Framework.Rectangle(chunk2.xSpriteSheet.Value, chunk2.ySpriteSheet.Value, Math.Min(d.sizeOfSourceRectSquares.Value, d.spriteChunkSheet.Bounds.Width), Math.Min(d.sizeOfSourceRectSquares.Value, d.spriteChunkSheet.Bounds.Height)), d.nonSpriteChunkColor.Value * chunk2.alpha, chunk2.rotation, new Vector2(d.sizeOfSourceRectSquares.Value / 2, d.sizeOfSourceRectSquares.Value / 2), chunk2.scale, SpriteEffects.None, ((float)(d.chunkFinalYLevel + 16) + position2.X / 10000f) / 10000f);
					}
					continue;
				}
				}
				if (d.itemId.Value != null)
				{
					...
					{
						Chunk chunk4 = d.Chunks[k];
						Vector2 position6 = chunk4.GetVisualPosition();
						Microsoft.Xna.Framework.Rectangle sourceRect2 = ((d.debrisType.Value == Debris.DebrisType.RESOURCE) ? itemData.GetSourceRect(chunk4.randomOffset) : itemData.GetSourceRect());
						SpriteEffects spriteEffect = ((d.floppingFish.Value && chunk4.bounces % 2 == 0) ? SpriteEffects.FlipHorizontally : SpriteEffects.None);
						b.Draw(texture, Utility.snapDrawPosition(Game1.GlobalToLocal(Game1.viewport, position6)), sourceRect2, Color.White, 0f, Vector2.Zero, scale, spriteEffect, ((float)(d.chunkFinalYLevel + 32) + position6.X / 10000f) / 10000f);
						b.Draw(Game1.shadowTexture, Utility.snapDrawPosition(Game1.GlobalToLocal(Game1.viewport, new Vector2(position6.X + 25.6f, (d.chunksMoveTowardPlayer ? (position6.Y + 8f) : ((float)d.chunkFinalYLevel)) + 32f + (float)(12 * d.itemQuality)))), Game1.shadowTexture.Bounds, Color.White * 0.75f, 0f, new Vector2(Game1.shadowTexture.Bounds.Center.X, Game1.shadowTexture.Bounds.Center.Y), Math.Min(3f, 3f - (d.chunksMoveTowardPlayer ? 0f : (((float)d.chunkFinalYLevel - position6.Y) / 96f))), SpriteEffects.None, (float)d.chunkFinalYLevel / 10000f);
					}
				}
				else
				{
					...
					{
						Vector2 position4 = Utility.snapDrawPosition(Game1.GlobalToLocal(Game1.viewport, d.Chunks[j].position.Value));
						...
						b.Draw(Game1.debrisSpriteSheet, position4, sourceRect, d.chunksColor.Value, 0f, Vector2.Zero, 4f * d.scale.Value, SpriteEffects.None, drawLayer);
					}
				}
			}
		}
		...
		public bool buildStructure(Building building, Vector2 tileLocation, Farmer who, bool skipSafetyChecks = false)
		{
			if (!skipSafetyChecks)
			{
				for (int y = 0; y < building.tilesHigh.Value; y++)
				{
					...
					{
						pokeTileForConstruction(new Vector2(tileLocation.X + (float)x3, tileLocation.Y + (float)y));
					}
				}
			...
			for (int y3 = 0; y3 < building.tilesHigh.Value; y3++)
			{
				for (int x = 0; x < building.tilesWide.Value; x++)
				{
					Vector2 currentGlobalTilePosition = new Vector2(tileLocation.X + (float)x, tileLocation.Y + (float)y3);
					if (!(terrainFeatures.GetValueOrDefault(currentGlobalTilePosition) is Flooring) || !(building.GetData()?.AllowsFlooringUnderneath ?? false))
					{
			...
			for (int y2 = 0; y2 < building.tilesHigh.Value; y2++)
			{
				...
				{
					RemoveArtifactSpots(new Vector2(tileLocation.X + (float)x2, tileLocation.Y + (float)y2));
				}
			}
				...
				for (int y = startingY; y < startingY + height; y++)
				{
					...
					if (TryGetTreeIdForTile(t, out var treeId, out var growthStageOnLoad, out var _, out var isFruitTree))
					{
						if (GetFurnitureAt(tile) == null && !terrainFeatures.ContainsKey(tile) && !objects.ContainsKey(tile))
						{
							if (isFruitTree)
							{
								terrainFeatures.Add(tile, new FruitTree(treeId, growthStageOnLoad.GetValueOrDefault(4)));
							}
							else
							{
								terrainFeatures.Add(tile, new Tree(treeId, growthStageOnLoad.GetValueOrDefault(5)));
							}
						}
						continue;
					}
