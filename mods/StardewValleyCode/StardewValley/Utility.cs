...
namespace StardewValley
{
	...
	public class Utility
	{
		...
		public static Event getWeddingEvent(Farmer farmer)
		{
			...
			bool ParseWeddingToken(string[] query, out string replacement, Random random, Farmer player)
			{
				...
				case "contextualweddingcelebrations":
				{
					...
					{
						if (attendee3.Celebration != null)
						{
							sb.Append(attendee3.Celebration);
							sb.Append("/");
						}
					}
		...
		public static float getFishCaughtPercent(Farmer who = null)
		{
			...
			foreach (ParsedItemData data in ItemRegistry.GetObjectTypeDefinition().GetAllData())
			{
				if (data.ObjectType == "Fish" && (!(data.RawData is ObjectData objData) || !objData.ExcludeFromFishingCollection))
				{
					totalFish += 1f;
		...
		public static Color? StringToColor(string rawColor)
		{
			...
			if (rawColor.StartsWith('#'))
			{
				...
				if ((rawColor.Length == 7 || rawColor.Length == 9) && byte.TryParse(rawColor.Substring(1, 2), NumberStyles.HexNumber, null, out var red2) && byte.TryParse(rawColor.Substring(3, 2), NumberStyles.HexNumber, null, out var green2) && byte.TryParse(rawColor.Substring(5, 2), NumberStyles.HexNumber, null, out var blue2) && (rawColor.Length == 7 || byte.TryParse(rawColor.Substring(7, 2), NumberStyles.HexNumber, null, out alpha)))
				{
			...
			else if (rawColor.Contains(' '))
			{
				...
				if ((parts.Length == 3 || parts.Length == 4) && ArgUtility.TryGetInt(parts, 0, out var red, out var error, "int red") && ArgUtility.TryGetInt(parts, 1, out var green, out error, "int green") && ArgUtility.TryGetInt(parts, 2, out var blue, out error, "int blue") && ArgUtility.TryGetOptionalInt(parts, 3, out var alpha2, out error, 255, "int alpha"))
				{
		...
		public static void drawLightningBolt(Vector2 strikePosition, GameLocation l)
		{
			...
			while (drawPosition.Y > (float)(-lightningSourceRect.Height * 4))
			{
				...
				TemporaryAnimatedSprite temporaryAnimatedSprite = new TemporaryAnimatedSprite("LooseSprites\\Cursors", lightningSourceRect, 9999f, 1, 999, drawPosition, flicker: false, Game1.random.NextBool(), (strikePosition.Y + 32f) / 10000f + 0.001f, 0.025f, Color.White, 4f, 0f, 0f, 0f);
				...
				defaultInterpolatedStringHandler.AppendFormatted(strikePosition.X);
				defaultInterpolatedStringHandler.AppendLiteral("_");
				defaultInterpolatedStringHandler.AppendFormatted(strikePosition.Y);
				defaultInterpolatedStringHandler.AppendLiteral("_");
		...
		public static float ApplyQuantityModifiers(float value, IList<QuantityModifier> modifiers, QuantityModifier.QuantityModifierMode mode = QuantityModifier.QuantityModifierMode.Stack, GameLocation location = null, Farmer player = null, Item targetItem = null, Item inputItem = null, Random random = null)
		{
			...
			foreach (QuantityModifier modifier in modifiers)
			{
				...
				default:
					newValue = QuantityModifier.Apply(newValue.GetValueOrDefault(value), modifier.Modification, amount);
					break;
				}
			}
		...
		public static FarmEvent pickPersonalFarmEvent()
		{
			...
			else
			{
				if (isMarriedOrRoommates)
				{
					...
					if (flag.HasValue && flag.GetValueOrDefault() && Game1.player.currentLocation == Game1.getLocationFromName(Game1.player.homeLocation.Value) && r.NextDouble() < 0.05 && GameStateQuery.CheckConditions(npcSpouse.GetData()?.SpouseWantsChildren))
					{
				...
				if (Game1.objectData.TryGetValue(geode.ItemId, out var data))
				{
					...
					if (geodeDrops != null && geodeDrops.Count > 0 && (!data.GeodeDropsDefaultItems || r.NextBool()))
					{
						foreach (ObjectGeodeDropData drop in data.GeodeDrops.OrderBy((ObjectGeodeDropData p) => p.Precedence))
						{
							...
							ObjectGeodeDropData data2 = drop;
							...
							defaultInterpolatedStringHandler.AppendLiteral("'");
							Item item = ItemQueryResolver.TryResolveRandomItem(data2, new ItemQueryContext(null, null, r, defaultInterpolatedStringHandler.ToStringAndClear()), avoidRepeat: false, null, null, null, delegate(string query, string error)
							{
								IGameLogger log = Game1.log;
								...
								log.Error(defaultInterpolatedStringHandler2.ToStringAndClear());
							});
							if (item != null)
							{
		...
		public static Pet findPet(Guid guid)
		{
			foreach (NPC character in Game1.getFarm().characters)
			{
				if (character is Pet pet2 && pet2.petId.Value.Equals(guid))
				{
					return pet2;
				}
			}
