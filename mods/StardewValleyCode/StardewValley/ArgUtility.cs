	...
	public static class ArgUtility
	{
		...
		public static bool TryGetEnum<TEnum>(string[] array, int index, out TEnum value, out string error, [CallerArgumentExpression("value")] string name = null) where TEnum : struct
		{
			...
			if (!Utility.TryParseEnum<TEnum>(raw, out value))
			{
				...
				error = GetValueParseError(array, index, name, required: true, defaultInterpolatedStringHandler.ToStringAndClear());
				return false;
			}
			error = null;
			return true;
		...
		public static bool TryGetVector2(string[] array, int index, out Vector2 value, out string error, bool integerOnly = false, [CallerArgumentExpression("value")] string name = null)
		{
			...
			if (integerOnly)
			{
				if (TryGetInt(array, index, out var x2, out error, xName) && TryGetInt(array, index + 1, out var y2, out error, yName))
				{
					value = new Vector2(x2, y2);
					return true;
				}
			}
			else if (TryGetFloat(array, index, out x, out error, xName) && TryGetFloat(array, index + 1, out y, out error, yName))
			{
		...
		internal static string GetMissingRequiredIndexError(string[] array, int index, string name)
		{
			...
			{
			case 0:
				return "required " + GetFieldLabel(index, name) + " not found (list is empty)";
			case 1:
				return "required " + GetFieldLabel(index, name) + " not found (list has a single value at index 0)";
			default:
			{
				...
				defaultInterpolatedStringHandler.AppendFormatted(GetFieldLabel(index, name));
