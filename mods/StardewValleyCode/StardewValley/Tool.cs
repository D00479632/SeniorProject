...
namespace StardewValley
{
	...
	public abstract class Tool : Item
	{
		...
		public virtual bool beginUsing(GameLocation location, int x, int y, Farmer who)
		{
			...
			else if (this is WateringCan wateringCan && wateringCan.WaterLeft <= 0)
			{
				Game1.toolAnimationDone(who);
				who.CanMove = true;
				who.canReleaseTool = false;
		...
		public virtual Object attach(Object o)
		{
			...
			for (int slot3 = 0; slot3 < attachments.Length; slot3++)
			{
				...
				{
					int toRemove = o.Stack - oldObj3.addToStack(o);
		...
		public virtual bool Forge(Item item, bool count_towards_stats = false)
		{
			...
			if (enchantment != null)
			{
				if (AddEnchantment(enchantment))
				{
					...
					{
						if (enchantment is GalaxySoulEnchantment && this is MeleeWeapon weapon2 && weapon2.isGalaxyWeapon() && weapon2.GetEnchantmentLevel<GalaxySoulEnchantment>() >= 3)
						{
			...
			else if (item.QualifiedItemId == "(O)852" && this is MeleeWeapon weapon)
			{
				...
				weapon.enchantments.RemoveWhere(delegate(BaseEnchantment curEnchantment)
				{
