		...
		protected virtual void parseDialogueString(string masterString, string translationKey)
		{
			...
			for (int i = 0; i < masterDialogueSplit.Length; i++)
			{
				...
				if (curDialogue.StartsWith('$'))
				{
					...
					if (length != 2)
					{
						if (length != 6)
						{
							...
							{
								if (!TriggerActionManager.TryRunAction(commandArgs, out var error2, out var exception))
								{
									...
									defaultInterpolatedStringHandler2.AppendLiteral(".");
									error2 = defaultInterpolatedStringHandler2.ToStringAndClear();
									if (exception == null)
									{
										Game1.log.Warn(error2);
									}
									else
									{
										Game1.log.Error(error2, exception);
									}
								}
							}));
						}
				...
				for (int l = 1; l < rawSplit.Length; l += 2)
				{
					...
					playerResponses.Add(new NPCDialogueResponse(null, -1, "quickResponse" + l, Game1.parseText(choice)));
