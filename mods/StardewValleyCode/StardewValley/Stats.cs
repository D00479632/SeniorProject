...
namespace StardewValley
{
	public class Stats
	{
		...
		public void checkForFishingAchievements()
		{
			...
			{
				if (itemData.ObjectType == "Fish" && (!(itemData.RawData is ObjectData objData) || !objData.ExcludeFromFishingCollection))
				{
					totalKindsOfFish++;
		...
		public void checkForFriendshipAchievements()
		{
			...
			foreach (KeyValuePair<string, string> pair2 in CraftingRecipe.cookingRecipes)
			{
			...
			foreach (KeyValuePair<string, string> pair in CraftingRecipe.craftingRecipes)
			{
