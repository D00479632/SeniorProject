	...
	public class Game1 : InstanceGame
	{
		...
		public static bool HasDedicatedHost
		{
			get
			{
				if (multiplayerMode != 0)
				{
					return (player?.team?.hasDedicatedHost.Value).GetValueOrDefault();
				}
				return false;
			}
		}
		...
		public void Instance_Initialize()
		{
			...
			defaultInterpolatedStringHandler.AppendFormatted(stopwatch.Elapsed);
			defaultInterpolatedStringHandler.AppendLiteral("'");
		...
		private IEnumerator<int> GetLoadContentEnumerator()
		{
			...
			for (int j = 0; j < dynamicPixelRects.Length; j++)
			{
				Texture2D[] array = dynamicPixelRects;
				int num2 = j;
				Texture2D texture2D = new Texture2D(base.GraphicsDevice, 1, 1, mipmap: false, SurfaceFormat.Color);
				...
				dynamicPixelRects[j].SetData(white);
			}
		...
		public static void performTenMinuteClockUpdate()
		{
			hooks.OnGame1_PerformTenMinuteClockUpdate(delegate
			{
				...
				case 2600:
					...
					{
						if (player.UsingTool && player.CurrentTool != null && player.CurrentTool is FishingRod fishingRod2 && fishingRod2.fishCaught)
						{
		...
		private static IEnumerator<int> _newDayAfterFade()
		{
			...
			if (IsMasterGame)
			{
				...
				seed = Utility.CreateRandomSeed(uniqueIDForThisGame / 100uL, stats.DaysPlayed * 10 + 1, stats.StepsTaken);
			...
			for (int k = player.enchantments.Count - 1; k >= 0; k--)
			{
				player.enchantments[k].OnUnequip(player);
			}
			...
			if (IsMasterGame)
			{
				Utility.ForEachLocation(delegate(GameLocation location)
				{
					...
					{
						if (value is Chest chest && chest.SpecialChestType == Chest.SpecialChestTypes.MiniShippingBin)
						{
							chest.clearNulls();
			...
			if (player.useSeparateWallets && player.IsMainPlayer)
			{
				foreach (Farmer who2 in getOfflineFarmhands())
				{
					...
					player.team.AddIndividualMoney(who2, total2);
			...
			if (IsMasterGame)
			{
				foreach (Farmer who in getAllFarmers())
				{
					if (who.isActive() && who.divorceTonight.Value && who.getSpouse() != null)
					{
			...
			if (IsMasterGame)
			{
				...
				foreach (NPC m in Utility.getAllCharacters())
				{
					if (m.IsVillager)
					{
						m.islandScheduleName.Value = null;
						m.currentScheduleDelay = 0f;
					}
					m.dayUpdate(dayOfMonth);
				}
		...
		public static void drawDialogue(NPC speaker)
		{
			...
			if (activeClickableMenu is DialogueBox dialogueBox && dialogueBox.dialogueFinished)
			{
				activeClickableMenu = null;
				return;
			}
		...
		public static void warpFarmer(LocationRequest locationRequest, int tileX, int tileY, int facingDirectionAfterWarp)
		{
			...
			case "Farm":
				...
				case "Forest":
				{
					...
					if (getFarm().TryGetMapPropertyAs("ForestEntry", out Point tile2, required: false))
					{
						tileX = tile2.X;
						tileY = tile2.Y;
						break;
				...
				case "BusStop":
				{
					if (tileX == 79 && tileY == 17 && getFarm().TryGetMapPropertyAs("BusStopEntry", out Point tile3, required: false))
					{
						tileX = tile3.X;
						tileY = tile3.Y;
					}
				...
				case "Backwoods":
				{
					if (tileX == 40 && tileY == 0 && getFarm().TryGetMapPropertyAs("BackwoodsEntry", out Point tile4, required: false))
					{
						tileX = tile4.X;
						tileY = tile4.Y;
					}
		...
		private static void notifyServerOfWarp(bool needsLocationInfo)
		{
			if (locationRequest != null)
			{
				byte flags = (byte)((locationRequest.IsStructure ? 1u : 0u) | (warpingForForcedRemoteEvent ? 2u : 0u) | (needsLocationInfo ? 4u : 0u));
		...
		{
			if (activeClickableMenu is ReadyCheckDialog readyCheckDialog && readyCheckDialog.checkName == "sleep" && !readyCheckDialog.isCancelable())
			{
		...
		public static void toolAnimationDone(Farmer who)
		{
			...
			if (who.Stamina > 0f)
			{
				...
				if (who.CurrentTool is FishingRod rod && rod.isFishing)
				{
					...
					else
					{
						if (((useToolButtonPressed && !isAnyGamePadButtonBeingPressed()) || (actionButtonPressed && isAnyGamePadButtonBeingPressed())) && pauseTime <= 0f && wasMouseVisibleThisFrame)
						{
							...
							foreach (IClickableMenu current2 in onScreenMenus)
							{
								...
								{
									if ((!IsChatting || current2 == chatBox) && (!(current2 is LevelUpMenu levelUpMenu) || levelUpMenu.informationUp) && current2.isWithinBounds(getMouseX(), getMouseY()))
									{
						...
						else
						{
							...
							if (actionButtonPressed || (dialogueUp && useToolButtonPressed))
							{
								...
								{
									if (wasMouseVisibleThisFrame && (IsHudDrawn || current4 == chatBox) && current4.isWithinBounds(getMouseX(), getMouseY()) && (!(current4 is LevelUpMenu levelUpMenu2) || levelUpMenu2.informationUp))
									{
			...
			if (hooks.OnRendering(RenderSteps.World, spriteBatch, time, target_screen))
			{
				...
				if (!currentLocation.shouldHideCharacters())
				{
					if (CurrentEvent == null)
					{
						...
						{
							if (!m.swimming.Value && !m.HideShadow && !m.IsInvisible && !checkCharacterTilesForShadowDrawFlag(m))
							{
					...
					else
					{
						...
						{
							if ((CurrentEvent == null || !CurrentEvent.ShouldHideCharacter(n)) && !n.swimming.Value && !n.HideShadow && !checkCharacterTilesForShadowDrawFlag(n))
							{
				...
				if (hooks.OnRendering(RenderSteps.World_Sorted, spriteBatch, time, target_screen))
				{
					if (!currentLocation.shouldHideCharacters())
					{
						if (CurrentEvent == null)
						{
							...
							{
								if (!n2.swimming.Value && !n2.HideShadow && !n2.isInvisible.Value && checkCharacterTilesForShadowDrawFlag(n2))
								{
						...
						else
						{
							...
							{
								if ((CurrentEvent == null || !CurrentEvent.ShouldHideCharacter(n3)) && !n3.swimming.Value && !n3.HideShadow && checkCharacterTilesForShadowDrawFlag(n3))
								{
		...
		public static void drawDialogueBox(int x, int y, int width, int height, bool speaker, bool drawOnlyBox, string message = null, bool objectDialogueWithPortrait = false, bool ignoreTitleSafe = true, int r = -1, int g = -1, int b = -1)
		{
			...
			if ((objectDialogueWithPortrait && objectDialoguePortraitPerson != null) || (speaker && currentSpeaker != null && currentSpeaker.CurrentDialogue.Count > 0 && currentSpeaker.CurrentDialogue.Peek().showPortrait))
			{
				...
				text2 = ((!objectDialogueWithPortrait) ? theSpeaker.CurrentDialogue.Peek().CurrentEmotion : ((objectDialoguePortraitPerson.Name == player.spouse) ? "$l" : "$neutral"));
				if (text2 != null)
				{
					int length = text2.Length;
					if (length != 2)
					{
						if (length == 8 && text2 == "$neutral")
						{
							goto IL_0705;
						}
					}
			...
			if (!(currentTool is Slingshot) && !(currentTool is Shears) && !(currentTool is MilkPail) && !(currentTool is Pan))
			{
				...
				case 1:
				{
					if (farmerSprite.currentAnimationIndex > 2)
					{
						...
						tileLocation2.X++;
						tileLocation2.Y--;
						if (!(f.CurrentTool is WateringCan) && f.currentLocation.hasTileAt(tileLocation2, "Front"))
						{
							return;
						}
						tileLocation2.Y++;
					...
					if (!(currentTool is FishingRod rod))
					{
						if (currentTool is WateringCan)
						{
							if (farmerSprite.currentAnimationIndex == 1)
							{
								...
								tileLocation.X--;
								tileLocation.Y--;
								if (f.currentLocation.hasTileAt(tileLocation, "Front") && f.Position.Y % 64f < 32f)
								{
									return;
								}
							}
				...
				case 3:
				{
					if (farmerSprite.currentAnimationIndex > 2)
					{
						...
						tileLocation4.X--;
						tileLocation4.Y--;
						if (!(f.CurrentTool is WateringCan) && f.currentLocation.hasTileAt(tileLocation4, "Front") && f.Position.Y % 64f < 32f)
						{
							return;
						}
						tileLocation4.Y++;
					...
					if (!(currentTool is FishingRod rod2))
					{
						if (currentTool is WateringCan)
						{
							if (farmerSprite.currentAnimationIndex == 1)
							{
								...
								tileLocation3.X--;
								tileLocation3.Y--;
								if (f.currentLocation.hasTileAt(tileLocation3, "Front") && f.Position.Y % 64f < 32f)
								{
									return;
								}
							}
				...
				if (farmerSprite.currentAnimationIndex > 2 && (!(f.CurrentTool is FishingRod rod3) || rod3.isCasting || rod3.castedButBobberStillInAir || rod3.isTimingCast))
				{
					Point tileLocation6 = f.TilePoint;
					if (f.currentLocation.hasTileAt(tileLocation6, "Front") && f.Position.Y % 64f < 32f && f.Position.Y % 64f > 16f)
					{
						return;
					}
				}
				...
				if (farmerSprite.currentAnimationIndex <= 2)
				{
					...
					tileLocation5.Y--;
					if (f.currentLocation.hasTileAt(tileLocation5, "Front"))
					{
						return;
					}
				}
		...
		public static void UpdateHorseOwnership()
		{
			...
			{
				if (stable5.owner.Value == -6666666 && GetPlayer(-6666666L) == null)
				{
		...
		protected virtual void _RecursiveRemoveThisNormalItemLocation(GameLocation l, string itemId)
		{
			...
			if (fridge != null)
			{
				...
				for (int j = 0; j < fridgeItems.Count; j++)
				{
					Item item3 = fridgeItems[j];
		...
		public string takeMapScreenshot(float? in_scale, string screenshot_name, Action onDone)
		{
			...
			if (string.IsNullOrWhiteSpace(screenshot_name))
			{
				...
				string text = SaveGame.FilterFileName(player.name.Value);
				DefaultInterpolatedStringHandler defaultInterpolatedStringHandler = new DefaultInterpolatedStringHandler(4, 4);
				defaultInterpolatedStringHandler.AppendLiteral("_");
				defaultInterpolatedStringHandler.AppendFormatted(now.Month);
				defaultInterpolatedStringHandler.AppendLiteral("-");
				defaultInterpolatedStringHandler.AppendFormatted(now.Day);
				defaultInterpolatedStringHandler.AppendLiteral("-");
				defaultInterpolatedStringHandler.AppendFormatted(now.Year);
				defaultInterpolatedStringHandler.AppendLiteral("_");
				defaultInterpolatedStringHandler.AppendFormatted((int)now.TimeOfDay.TotalMilliseconds);
				screenshotName = text + defaultInterpolatedStringHandler.ToStringAndClear();
			}
		...
		private static void GetScreenshotRegion(GameLocation screenshotLocation, out int startX, out int startY, out int width, out int height)
		{
			...
			{
				log.Error("GetScreenshotRegion failed with exception:", ex);
			}
		}
	}
}
