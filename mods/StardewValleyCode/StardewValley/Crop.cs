	...
	public class Crop : INetObject<NetFields>, IHaveModData
	{
		...
		public virtual bool harvest(int xTile, int yTile, HoeDirt soil, JunimoHarvester junimoHarvester = null, bool isForcedScytheHarvest = false)
		{
			...
			else if (currentPhase.Value >= phaseDays.Count - 1 && (!fullyGrown.Value || dayOfCurrentPhase.Value <= 0))
			{
				...
				cropQuality = MathHelper.Clamp(cropQuality, data?.HarvestMinQuality ?? 0, (data?.HarvestMaxQuality).GetValueOrDefault(cropQuality));
				int numToHarvest = 1;
				if (data != null)
				{
		...
		public virtual bool TryGrowGiantCrop(bool checkPreconditions = true, Random random = null)
		{
			...
			foreach (KeyValuePair<string, GiantCropData> pair in possibleGiantCrops)
			{
				...
				bool valid = true;
				for (int y2 = (int)tile.Y; (float)y2 < tile.Y + (float)giantCrop.TileSize.Y; y2++)
				{
					for (int x2 = (int)tile.X; (float)x2 < tile.X + (float)giantCrop.TileSize.X; x2++)
					{
						if (!(environment.terrainFeatures.GetValueOrDefault(new Vector2(x2, y2)) is HoeDirt dirt) || !(dirt.crop?.indexOfHarvest.Value == indexOfHarvest.Value))
						{
							valid = false;
							break;
						}
