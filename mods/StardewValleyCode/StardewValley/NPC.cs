...
namespace StardewValley
{
	...
	public class NPC : Character, IComparable
	{
		...
		public void performSpecialScheduleChanges()
		{
			...
			foreach (KeyValuePair<int, SchedulePathDescription> v in Schedule)
			{
				...
				case "IslandSouth":
				{
					...
					Object sign2 = ItemRegistry.Create<Object>("(BC)TextSign");
					sign2.signText.Value = TokenStringBuilder.LocalizedText((v.Value.targetLocationName == "IslandSouth") ? "Strings\\1_6_Strings:Pam_busSign_resort" : "Strings\\1_6_Strings:Pam_busSign");
					sign2.SpecialVariable = 987659;
					busStop.tryPlaceObject(new Vector2(25f, 10f), sign2);
					foundBus = true;
					pamGone = true;
					break;
				}
			...
			if (!foundBus && !Game1.isGreenRain)
			{
				...
				sign.signText.Value = TokenStringBuilder.LocalizedText("Strings\\1_6_Strings:Pam_busSign_generic");
		...
		public virtual bool tryToReceiveActiveObject(Farmer who, bool probe = false)
		{
			...
			string text = who.ActiveObject?.QualifiedItemId;
			if (text != null)
			{
				switch (text.Length)
				{
				case 6:
					...
					{
					case '3':
						if (!(text == "(O)233") || !(name.Value == "Jas") || Utility.GetDayOfPassiveFestival("DesertFestival") <= 0 || !(base.currentLocation is Desert) || who.mailReceived.Contains("Jas_IceCream_DF_" + Game1.year))
						{
							break;
						}
						if (!probe)
					...
					case '0':
						...
						if (who.hasQuest("130"))
						{
							Dialogue dialogue5 = TryGetDialogue("accept_" + activeObj.ItemId);
							if (dialogue5 != null)
							{
								if (!probe)
								{
									setNewDialogue(dialogue5);
									Game1.drawDialogue(this);
							...
							dialogue5 = TryGetDialogue("reject_" + activeObj.ItemId);
							if (dialogue5 != null)
							{
								if (!probe)
								{
			...
			Dialogue dialogue4 = TryGetDialogue("RejectItem_" + activeObj.QualifiedItemId) ?? (from tag in activeObj.GetContextTags()
				...
				if (canReceiveGifts)
				{
					...
					if (friendship.IsDating())
					{
						if (!probe)
						{
							...
							defaultInterpolatedStringHandler.AppendFormatted(friendship.Status);
							Dialogue dialogue = TryGetDialogue(defaultInterpolatedStringHandler.ToStringAndClear()) ?? TryGetDialogue("RejectBouquet_AlreadyAccepted");
							if (dialogue != null)
							{
			...
			case "(O)460":
				if (canReceiveGifts)
				{
					...
					if (who.spouse == base.Name)
					{
						...
						defaultInterpolatedStringHandler.AppendFormatted(friendship?.Status);
						Dialogue dialogue2 = TryGetDialogue(defaultInterpolatedStringHandler.ToStringAndClear()) ?? TryGetDialogue("RejectMermaidPendant_AlreadyAccepted");
						if (!probe && dialogue2 != null)
						{
							CurrentDialogue.Push(dialogue2);
							Game1.drawDialogue(this);
						}
						return dialogue2 != null;
					}
		...
		public bool checkForNewCurrentDialogue(int heartLevel, bool noPreface = false)
		{
			...
			Dialogue dialogue = TryGetDialogue(preface + Game1.currentLocation.name.Value + "_" + base.TilePoint.X + "_" + base.TilePoint.Y) ?? TryGetDialogue(preface + Game1.currentLocation.name.Value + "_" + Game1.shortDayNameFromDayOfSeason(Game1.dayOfMonth));
			int hearts = 10;
			while (dialogue == null && hearts >= 2)
			{
				if (heartLevel >= hearts)
				{
					dialogue = TryGetDialogue(preface + Game1.currentLocation.name.Value + hearts);
				}
				hearts -= 2;
			}
		...
		private void setNewDialogue(string dialogueSheetName, string dialogueSheetKey, bool clearOnMovement = false)
		{
			...
			if (dialogueSheetName.Contains("Marriage"))
			{
				...
				{
					Dialogue dialogue2 = tryToGetMarriageSpecificDialogue(translationKey);
					if (dialogue2 == null)
					{
						IGameLogger log = Game1.log;
		...
		public virtual SchedulePathDescription pathfindToNextScheduleLocation(string scheduleKey, string startingLocation, int startingX, int startingY, string endingLocation, int endingX, int endingY, int finalFacingDirection, string endBehavior, string endMessage)
		{
			...
			if (locationsRoute != null)
			{
				for (int i = 0; i < locationsRoute.Length; i++)
				{
					...
					{
						if (Utility.TryGetPassiveFestivalData(activePassiveFestival, out var data2) && data2.MapReplacements != null && data2.MapReplacements.TryGetValue(targetLocationName2, out var newName2))
						{
		...
		protected virtual Dictionary<int, SchedulePathDescription> parseMasterScheduleImpl(string scheduleKey, string rawData, List<string> visited)
		{
			...
			try
			{
				...
				if (split[0].Contains("GOTO"))
				{
					...
					if (newKey3.EqualsIgnoreCase("season"))
					{
					...
					try
					{
						if (allSchedules.TryGetValue(newKey3, out var newScript))
						{
				...
				if (split[0].Contains("NOT"))
				{
					...
					if (commandSplit[1].ToLower() == "friendship")
					{
						...
						for (; index2 < commandSplit.Length; index2 += 2)
						{
							...
							if (int.TryParse(commandSplit[index2 + 1], out var level))
							{
			...
			else if (!Game1.isRaining && r.NextDouble() < 0.4 && !checkTileOccupancyForSpouse(farm, Utility.PointToVector2(farmHouse.getPorchStandingSpot())) && !base.Name.Equals("Krobus"))
			{
				...
				if (r.NextDouble() < 0.6 && Game1.season != Season.Winter && !hasSomeoneWateredCrops)
				{
					...
					for (; tries2 < Math.Min(50, farm.terrainFeatures.Length); tries2++)
					{
						...
						if (Utility.TryGetRandom(farm.terrainFeatures, out var tile2, out var feature) && feature is HoeDirt dirt2 && dirt2.needsWatering())
						{
							if (!dirt2.isWatered())
							{
								origin2 = tile2;
					...
					if (!origin2.Equals(Vector2.Zero))
					{
						...
						{
							if (farm.isTileOnMap(currentPosition2) && farm.terrainFeatures.TryGetValue(currentPosition2, out var terrainFeature) && terrainFeature is HoeDirt dirt && Game1.IsMasterGame && dirt.needsWatering())
							{
								dirt.state.Value = 1;
							}
						}
				...
				else if (!hasSomeoneRepairedTheFences)
				{
					...
					for (; tries < Math.Min(50, farm.objects.Length); tries++)
					{
						...
						if (Utility.TryGetRandom(farm.objects, out var tile, out var obj2) && obj2 is Fence)
						{
							origin = tile;
						}
					}
