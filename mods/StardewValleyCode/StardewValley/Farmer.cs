...
namespace StardewValley
{
	public class Farmer : Character, IComparable
	{
		...
		public override int FacingDirection
		{
			get
			{
				if (!IsLocalPlayer && !isFakeEventActor && UsingTool && CurrentTool is FishingRod rod && rod.CastDirection >= 0)
				{
		...
		public void AddMissedMailAndRecipes()
		{
			...
			{
				if (CraftingRecipe.TryParseLevelRequirement(v2.Key, v2.Value, isCooking: true, out var reqSkillNumber2, out var minLevel2) && GetUnmodifiedSkillLevel(reqSkillNumber2) >= minLevel2)
				{
					cookingRecipes.TryAdd(v2.Key, 0);
					addRobinKitchenLetter = true;
				}
			}
			foreach (KeyValuePair<string, string> v in CraftingRecipe.craftingRecipes)
			{
				if (CraftingRecipe.TryParseLevelRequirement(v.Key, v.Value, isCooking: false, out var reqSkillNumber, out var minLevel) && GetUnmodifiedSkillLevel(reqSkillNumber) >= minLevel)
				{
					craftingRecipes.TryAdd(v.Key, 0);
				}
			}
		...
		public void resetFriendshipsForNewDay()
		{
			foreach (string name in friendshipData.Keys)
			{
				...
				if (i != null)
				{
					if (i != null && i.datable.Value && !friendshipData[name].IsDating() && !i.isMarried())
					{
						single = true;
					}
		...
		public void addItemsByMenuIfNecessary(List<Item> itemsToAdd, ItemGrabMenu.behaviorOnItemSelect itemSelectedCallback = null, bool forceQueue = false)
		{
			...
			if (itemsToAdd.Count > 0 && (forceQueue || Game1.activeClickableMenu != null))
			{
				...
				{
					if (Game1.nextClickableMenu[menuIndex] is ItemGrabMenu menu && menu.source == 4)
					{
		...
		public void OnItemReceived(Item item, int countAdded, Item mergedIntoStack, bool hideHudNotification = false)
		{
			...
			if (actualItem is Object obj && obj.specialItem)
			{
			...
			if (qualifiedItemId != null)
			{
				...
				case 6:
					...
					case '5':
						...
						break;
					case '4':
						if (qualifiedItemId == "(O)428" && !hasOrWillReceiveMail("clothFound"))
						{
		...
		public static void drinkGlug(Farmer who)
		{
			...
			if (who.itemToEat != null)
			{
				string text = ArgUtility.SplitBySpace(who.itemToEat.Name).Last();
				if (text != null)
				{
					switch (text.Length)
					{
			...
			if (Game1.currentLocation == who.currentLocation)
			{
				Game1.playSound((who.itemToEat is Object o && o.preserve.Value.GetValueOrDefault() == Object.PreserveType.Pickle) ? "eat" : "gulp");
			}
		...
		public bool checkAction(Farmer who, GameLocation location)
		{
			...
			if (who.CanMove)
			{
				...
				if (flag.HasValue && flag.GetValueOrDefault() && !who.ActiveObject.questItem.Value)
				{
					who.Halt();
					who.faceGeneralDirection(getStandingPosition(), 0, opposite: false, useTileCalculations: false);
		...
		private void performDrinkAnimation(Object item)
		{
			...
			isEating = true;
			if (item != null && item.HasContextTag("mayo_item") && Utility.isThereAFarmerOrCharacterWithinDistance(base.Tile, 7, base.currentLocation) is NPC npc && npc.Age != 2)
			{
