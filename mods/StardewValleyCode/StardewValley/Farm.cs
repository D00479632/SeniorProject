...
namespace StardewValley
{
	public class Farm : GameLocation
	{
		...
		public override void DayUpdate(int dayOfMonth)
		{
			...
			if (ShouldSpawnBeachFarmForage())
			{
				while (Game1.random.NextDouble() < 0.9)
				{
					...
					if (!CanItemBePlacedHere(v2) || hasTileAt((int)v2.X, (int)v2.Y, "AlwaysFront"))
					{
						continue;
					}
			...
			if (ShouldSpawnForestFarmForage() && !Game1.IsWinter)
			{
				while (Game1.random.NextDouble() < 0.75)
				{
					...
					if (CanItemBePlacedHere(v, itemIsPassable: false, CollisionMask.All, CollisionMask.None) && !hasTileAt((int)v.X, (int)v.Y, "AlwaysFront") && ((Game1.whichFarm == 2 && v.X < 18f) || doesTileHavePropertyNoNull((int)v.X, (int)v.Y, "Type", "Back").Equals("Grass")))
					{
		...
		public void addCrows()
		{
			...
			{
				if (pair.Value is HoeDirt dirt2 && dirt2.crop != null)
				{
					numCrops++;
				}
			}
		...
		{
			if (item is Object obj && obj.QualifiedItemId == "(BC)160")
			{
