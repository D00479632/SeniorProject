		...
		private void checkForFootstep()
		{
			...
			if (((currentSingleAnimation >= 32 && currentSingleAnimation <= 56) || (currentSingleAnimation >= 128 && currentSingleAnimation <= 152)) && currentAnimationIndex % 4 == 0)
			{
				...
				if (!(played_step == "sandyStep"))
				{
					...
					{
						TemporaryAnimatedSprite sprite2 = TemporaryAnimatedSprite.GetTemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(247, 407, 6, 6), 2000f, 1, 10000, new Vector2(owner_position.X + 24f + (float)(Game1.random.Next(-4, 4) * 4), owner_position.Y + 8f + (float)(Game1.random.Next(-4, 4) * 4)), flicker: false, flipped: false, owner_position.Y / 10000000f, 0.01f, Color.White, 3f + (float)Game1.random.NextDouble(), 0f, (owner.FacingDirection == 1 || owner.FacingDirection == 3) ? (-(float)Math.PI / 4f) : 0f, 0f);
