	...
	public static class TriggerActionManager
	{
		...
		public static class DefaultActions
		{
			...
			public static bool If(string[] args, TriggerActionContext context, out string error)
			{
				...
				if (GameStateQuery.CheckConditions(ArgUtility.UnsplitQuoteAware(args, ' ', 1, startTrueIndex - 1 - 1)))
				{
					...
					string action2 = ArgUtility.UnsplitQuoteAware(args, ' ', startTrueIndex, maxCount);
				...
				{
					string action = ArgUtility.UnsplitQuoteAware(args, ' ', startFalseIndex);
		...
		public static CachedAction ParseAction(string action)
		{
			...
			if (!ActionCache.TryGetValue(action, out var parsed))
			{
				...
				if (!TryGetActionHandler(actionKey, out var handler))
				{
					...
					defaultInterpolatedStringHandler.AppendFormatted(string.Join("', '", ActionHandlers.Keys.OrderBy<string, string>((string p) => p, StringComparer.OrdinalIgnoreCase)));
					defaultInterpolatedStringHandler.AppendLiteral("')");
					cachedAction = new CachedAction(args, handler2, defaultInterpolatedStringHandler.ToStringAndClear(), isNullHandler: true);
				}
				else
				{
					cachedAction = new CachedAction(args, handler, null, isNullHandler: false);
				}
				parsed = cachedAction;
