...
namespace StardewValley.Tests
{
	...
	public class SyntaxAbstractor
	{
			...
			if (!(key == "ForestPylonEvent"))
			{
				if (key == "StarterChicken_Names")
				{
					...
					if (omittedPairs)
					{
						...
						defaultInterpolatedStringHandler.AppendFormatted(syntax);
		...
		private void ExtractEventSyntaxImpl(string text, ref int index, StringBuilder syntax, int maxIndex = -1)
		{
			...
			foreach (string rawCommand in array2)
			{
				...
				if (!string.IsNullOrWhiteSpace(rawCommand))
				{
					...
					int nextArg = 1;
					if (Event.TryResolveCommandName(commandName, out var actualName) && actualName != null)
					{
						...
						case 13:
							...
							case 'Q':
								if (actualName == "QuickQuestion")
								{
									...
									for (int j = 1; j < masterSplit.Length; j++)
									{
										masterSplit[j] = masterSplit[j].Replace('\\', '/');
										syntax.Append("(break)");
										int tempIndex = 0;
										ExtractEventSyntaxImpl(masterSplit[j], ref tempIndex, syntax);
									}
									syntax.Append('"');
									nextArg = args.Length;
								}
							...
							case 'l':
							{
								...
								if (dialogues != null)
								{
									...
									for (int i = 0; i < dialogues.Length; i++)
									{
										...
										AppendEventCommandDialogueArg(syntax, dialogues, i, prependSpace: false, quote: false);
									}
									syntax.Append('"');
								}
								nextArg = 3;
		...
		private void ExtractDialogueCommandSyntax(string text, ref int index, StringBuilder syntax, char commandDelimiter)
		{
			...
			string commandName = text.Substring(startIndex4, index - startIndex4);
			syntax.Append(commandName);
			if (commandName != null)
			{
			...
			if (commandName == "$y")
			{
				...
				while (true)
				{
					int nextIndex = text.IndexOf('\'', endIndex3 + 1);
			...
			index++;
			syntax.Append(text.Substring(startIndex, index - startIndex).TrimEnd(' '));
			int endIndex2;
			for (endIndex2 = index; endIndex2 < text.Length && text[endIndex2] != '#' && text[endIndex2] != '|'; endIndex2++)
			{
			}
			ExtractDialogueSyntaxImpl(text, commandDelimiter, ref index, syntax, endIndex2 - 1);
			if (index < text.Length && text[index] == '|')
			{
				syntax.Append(text[index]);
				index++;
				int endIndex;
				for (endIndex = index; endIndex < text.Length && text[endIndex] != '#' && text[endIndex] != '|'; endIndex++)
				{
				}
				ExtractDialogueSyntaxImpl(text, commandDelimiter, ref index, syntax, endIndex - 1);
				goto IL_03a0;
			}
			return;
			IL_03a0:
		...
		private void ExtractMailCommandSyntax(string text, ref int index, StringBuilder syntax)
		{
			...
			string commandName = text.Substring(startIndex2, index - startIndex2);
			if (!(commandName == "%item"))
			{
			...
			string command = ((text[index] == '%' && text[index - 1] == '%' && char.IsWhiteSpace(text[index - 2])) ? (text.Substring(startIndex, index - startIndex - 1).TrimEnd() + "%%") : text.Substring(startIndex, index - startIndex + 1));
