	...
	public class MeleeWeapon : Tool
	{
		...
		public virtual void FireProjectile(Farmer who)
		{
			...
			foreach (WeaponProjectile data in cachedData.Projectiles)
			{
				...
				if (data.Item != null)
				{
					GenericSpawnItemData item = data.Item;
					...
					shotItemId = ItemQueryResolver.TryResolveRandomItem(item, new ItemQueryContext(currentLocation, who, null, defaultInterpolatedStringHandler.ToStringAndClear()))?.QualifiedItemId;
					if (shotItemId == null)
					{
						continue;
					}
		...
		public override void drawTooltip(SpriteBatch spriteBatch, ref int x, ref int y, SpriteFont font, float alpha, StringBuilder overrideText)
		{
			...
			if (speed.Value != ((type.Value == 2) ? (-8) : 0))
			{
				...
				Utility.drawTextWithShadow(spriteBatch, Game1.content.LoadString("Strings\\UI:ItemHover_Speed", ((((type.Value == 2) ? (speed.Value - -8) : speed.Value) > 0) ? "+" : "") + ((type.Value == 2) ? (speed.Value - -8) : speed.Value) / 2), font, new Vector2(x + 16 + 52, y + 16 + 12), negativeSpeed ? Color.DarkRed : (c7 * 0.9f * alpha));
				y += (int)Math.Max(font.MeasureString("TT").Y, 48f);
			}
			if (addedDefense.Value > 0)
			{
