	...
	public class ParrotUpgradePerch : INetObject<NetFields>
	{
		...
		public static void ActivateGoldenParrot()
		{
			...
			foreach (GameLocation i in Game1.locations)
			{
				...
				{
					if (!(i is IslandWestCave1 cave))
					{
						if (!(i is IslandEast east))
						{
							if (!(i is IslandHut hut))
							{
								if (!(i is IslandSouthEast se))
								{
									if (!(i is IslandShrine shrine))
									{
										if (!(i is IslandWest west))
										{
											if (i is IslandFarmCave fcave)
											{
