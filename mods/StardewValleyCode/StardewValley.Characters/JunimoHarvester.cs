			...
			if (Game1.IsMasterGame)
			{
				if (harvestTimer > 0)
				{
					...
					else if (oldTimer >= 1000 && harvestTimer < 1000)
					{
						netAnimationEvent.Fire(2);
						JunimoHut hut2 = home;
						if (base.currentLocation != null && hut2 != null && !hut2.noHarvest.Value && isHarvestable())
						{
									...
									if (gotDouble)
									{
										for (int i = 0; i < 2; i++)
										{
											...
											if (lastItemHarvested is ColoredObject coloredObj2)
											{
												...
												Game1.multiplayer.broadcastSprites(base.currentLocation, new TemporaryAnimatedSprite(itemData.TextureName, colored_source_rect2, 1000f, 1, 0, base.Position + new Vector2(0f, -40f), flicker: false, flipped: false, mainDrawLayer + 0.005f, 0.02f, coloredObj2.color.Value, 4f, -0.01f, 0f, 0f)
												{
													motion = new Vector2((float)((i != 0) ? 1 : (-1)) * 0.5f, -0.25f),
													delayBeforeAnimationStart = 200
												});
				...
				else if (alpha > 0f && controller == null)
				{
					...
					if (Game1.random.NextDouble() < 0.002)
					{
						...
						{
							JunimoHut hut = home;
							if (hut != null && !hut.noHarvest.Value)
							{
								pathfindToNewCrop();
							}
							break;
