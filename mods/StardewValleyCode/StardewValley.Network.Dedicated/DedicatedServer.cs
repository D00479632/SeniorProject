		...
		private void CheckForWarpEvents(FarmerWarp warp)
		{
			...
			foreach (string eventKey in events.Keys)
			{
				...
				for (int i = 0; i < array.Length; i++)
				{
					...
					if (flag.HasValue && !flag.GetValueOrDefault() && Event.TryGetEventCommandHandler(commandName, out var eventCommandHandler) && (object)eventCommandHandler == broadcastEventCommand)
					{
						fakeFarmerId = warp.who.UniqueMultiplayerID;
						warpingHostBroadcastEvent = true;
