	...
	public class CollectionsPage : IClickableMenu
	{
			...
			: base(x, y, width, height)
		{
			...
			foreach (ParsedItemData data in dataEntries)
			{
				...
				case "Arch":
					whichCollection = 2;
					farmerHas2 = LibraryMuseum.HasDonatedArtifact(id);
					break;
				case "Fish":
					if (data.RawData is ObjectData objData && objData.ExcludeFromFishingCollection)
					{
						continue;
					}
					whichCollection = 1;
					farmerHas2 = Game1.player.fishCaught.ContainsKey(data.QualifiedItemId);
					break;
				default:
					if (data.Category != -2)
					{
						if (type == "Cooking" || data.Category == -7)
						{
							...
							{
								switch (last_minute_1_5_hack_name.Length)
								{
								case 15:
									switch (last_minute_1_5_hack_name[0])
									{
									case 'C':
										if (last_minute_1_5_hack_name == "Cranberry Sauce")
										{
								...
								case 16:
									...
									break;
								case 6:
									if (last_minute_1_5_hack_name == "Cookie")
									{
						...
						else
						{
							...
							whichCollection = 0;
							farmerHas2 = Game1.player.basicShipped.ContainsKey(id);
						}
						break;
					}
					goto case "Minerals";
		...
		public string createDescription(string id)
		{
			...
			default:
			{
				...
				case "Fish":
				{
					if (Game1.player.fishCaught.TryGetValue("(O)" + id, out var fields2))
					{
