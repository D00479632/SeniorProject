...
namespace StardewValley.Menus
{
	public class TitleMenu : IClickableMenu, IDisposable
	{
		...
		public void backButtonPressed()
		{
			...
			else if ((subMenu is TitleTextInputMenu titleTextInputMenu && titleTextInputMenu.context == "join_menu") || subMenu is FarmhandMenu)
			{
				buttonsDX = 0;
			...
			else if (subMenu is CharacterCustomization customizationMenu && customizationMenu.source == CharacterCustomization.Source.HostNewFarm)
			{
				buttonsDX = 0;
		...
		public override void receiveLeftClick(int x, int y, bool playSound = true)
		{
					...
					if (Game1.content.GetCurrentLanguage() != LocalizedContentManager.LanguageCode.zh)
					{
						...
						else if (cornerRect.Contains(x, y) && !cornerPhaseHolding)
						{
							...
							else
							{
								...
								for (int k = 0; k < 3; k++)
								{
									tempSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(280 + Game1.random.Choose(8, 0), 1954, 8, 8), 1000f, 1, 99, new Vector2(width / 2 - 190 * zoom3, -300 * zoom3 - (int)(viewportY / 3f) * zoom3 + 175 * zoom3), flicker: false, flipped: false, 1f, 0f, Color.White, pixelZoom, 0f, 0f, (float)Game1.random.Next(-10, 11) / 100f)
									{
										motion = new Vector2(Game1.random.Next(-4, 5), -8f + (float)Game1.random.Next(-10, 1) / 100f),
										acceleration = new Vector2(0f, 0.3f),
										local = true,
										delayBeforeAnimationStart = k * 15
									});
								}
							}
						}
			...
			{
				ClickableTextureComponent c = buttons[i];
		...
		public override void update(GameTime time)
		{
					...
					if (cornerClickParrotTimer <= 0f)
					{
						...
						for (int l = 0; l < 14; l++)
						{
							tempSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(355, 1199, 16, 16), new Vector2(globalXOffset + width / 2 - 220 * zoom, (float)(-300 * zoom) - viewportY / 3f * (float)zoom + (float)(60 * zoom) + (float)(Game1.random.Next(100) * zoom)), Game1.random.NextBool(), 0f, new Color(180, 180, 240))
							{
								...
								local = true,
								holdLastFrame = false,
								delayBeforeAnimationStart = 100 + l * 20
							});
						}
						for (int k = 0; k < 14; k++)
						{
							tempSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(355, 1199, 16, 16), new Vector2(globalXOffset + width / 2 + 220 * zoom, (float)(-300 * zoom) - viewportY / 3f * (float)zoom + (float)(60 * zoom) + (float)(Game1.random.Next(100) * zoom)), Game1.random.NextBool(), 0f, new Color(180, 180, 240))
							{
								...
								local = true,
								holdLastFrame = false,
								delayBeforeAnimationStart = 900 + k * 20
							});
						}
					}
				}
			}
			for (int j = bigClouds.Count - 1; j >= 0; j--)
			{
				bigClouds[j] -= 0.1f;
				bigClouds[j] += buttonsDX * time.ElapsedGameTime.Milliseconds / 2;
				if (bigClouds[j] < (float)(-512 * pixelZoom))
				{
					bigClouds[j] = width;
				}
			}
			for (int i = smallClouds.Count - 1; i >= 0; i--)
			{
				...
				if (smallClouds[i] < (float)(-149 * pixelZoom))
				{
					smallClouds[i] = width;
				}
			}
		...
		public override void draw(SpriteBatch b)
		{
			...
			{
				for (int x3 = -10; x3 < width; x3 += 638)
				{
					b.Draw(Game1.mouseCursors, new Vector2(x3 * pixelZoom, (float)(-360 * pixelZoom) - viewportY * 0.66f), new Rectangle(0, 1453, 638, 195), Color.White * (1f - (float)globalXOffset / 1200f), 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.8f);
				}
			}
			...
			else if (subMenu == null && isTransitioningButtons && (whichSubMenu.Equals("Load") || whichSubMenu.Equals("New")))
			{
				...
				int w = 0;
				int h = 64;
				Utility.makeSafe(ref x4, ref y2, w, h);
				SpriteText.drawStringWithScrollBackground(b, Game1.content.LoadString("Strings\\StringsFromCSFiles:Game1.cs.3689"), x4, y2);
			}
			...
			else if (logoFadeTimer > 0 || fadeFromWhiteTimer > 0)
			{
				...
				if (specialSurprised)
				{
					...
					for (int x2 = 64; x2 < width + 1000; x2 += 192)
					{
						...
						{
							b.Draw(Game1.mouseCursors, new Vector2(x2, y) + new Vector2((float)(-time) / 20f, (float)time / 20f), new Rectangle(355 + (time + x2 * 77 + y * 77) / 12 % 110 / 11 * 16, 1200, 16, 16), Color.White * 0.66f * ((float)(fadeFromWhiteTimer - (2000 - fadeFromWhiteTimer)) / 2000f), 0f, Vector2.Zero, pixelZoom, SpriteEffects.None, 0.18f);
						}
					}
