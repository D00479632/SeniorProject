...
namespace StardewValley.Menus
{
	public class ShopMenu : IClickableMenu
	{
		...
		public virtual void setUpStoreForContext()
		{
			...
			if (shopId != null)
			{
				...
				case 24:
				{
					...
					if (c != 'J')
					{
						if (c != 'W' || !(shopId == "WizardFurnitureCatalogue"))
						{
							break;
						}
					}
				...
				case 23:
				{
					...
					if (c != 'R')
					{
						if (c != 'T' || !(shopId == "TrashFurnitureCatalogue"))
						{
							break;
						}
					}
		...
		private bool tryToPurchaseItem(ISalable item, ISalable held_item, int stockToBuy, int x, int y)
		{
			...
			if (held_item == null)
			{
				...
				if (getPlayerCurrencyAmount(Game1.player, currency) >= price && (extraTradeItem == null || HasTradeItem(extraTradeItem, extraTradeItemCount)))
				{
					...
					{
						BuyBuybackItem(item, price, stacksToBuy2);
					}
			...
			else if (held_item.canStackWith(item))
			{
				...
				if (stockToBuy > 0)
				{
					...
					if (getPlayerCurrencyAmount(Game1.player, currency) >= price2 && (extraTradeItem2 == null || HasTradeItem(extraTradeItem2, extraTradeItemCount2)))
					{
						...
						if (CanBuyback() && buyBackItems.Contains(item))
						{
							BuyBuybackItem(item, price2, stacksToBuy);
						}
		...
		public override void draw(SpriteBatch b)
		{
			...
			{
				ClickableComponent forSaleButton = forSaleButtons[j];
				if (currentItemIndex + j >= forSale.Count)
				{
					continue;
				}
				bool failedCanPurchaseCheck = canPurchaseCheck != null && !canPurchaseCheck(currentItemIndex + j);
