		...
		public override void draw(SpriteBatch b)
		{
			...
			else
			{
				...
				case CarpentryAction.None:
				{
					...
					for (int y2 = 0; y2 < currentBuilding.tilesHigh.Value; y2++)
					{
						for (int x = 0; x < currentBuilding.tilesWide.Value; x++)
						{
							int sheetIndex = currentBuilding.getTileSheetIndexForStructurePlacementTile(x, y2);
					...
					foreach (BuildingPlacementTile additionalPlacementTile in currentBuilding.GetAdditionalPlacementTiles())
					{
						...
						foreach (Point point in additionalPlacementTile.TileArea.GetPoints())
						{
							int x2 = point.X;
							int y = point.Y;
							int sheetIndex2 = currentBuilding.getTileSheetIndexForStructurePlacementTile(x2, y);
				...
				case CarpentryAction.Move:
				{
					...
					for (int y4 = 0; y4 < buildingToMove.tilesHigh.Value; y4++)
					{
						for (int x3 = 0; x3 < buildingToMove.tilesWide.Value; x3++)
						{
							int sheetIndex3 = buildingToMove.getTileSheetIndexForStructurePlacementTile(x3, y4);
					...
					foreach (BuildingPlacementTile additionalPlacementTile2 in buildingToMove.GetAdditionalPlacementTiles())
					{
						...
						foreach (Point point2 in additionalPlacementTile2.TileArea.GetPoints())
						{
							int x4 = point2.X;
							int y3 = point2.Y;
							int sheetIndex4 = buildingToMove.getTileSheetIndexForStructurePlacementTile(x4, y3);
