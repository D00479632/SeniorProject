...
namespace StardewValley.Menus
{
	public class SpecialCurrencyDisplay
	{
		...
		public virtual void OnCurrencyChange(NetIntDelta field, int oldValue, int newValue)
		{
			...
			foreach (CurrencyRenderInfo render2 in displayedCurrencies)
			{
				if ((object)render2.currency.field == field)
				{
			...
			foreach (CurrencyDisplayType currency in registeredCurrencyDisplays.Values)
			{
				...
				{
					CurrencyRenderInfo render = new CurrencyRenderInfo(currency);
					render.OnCurrencyChanged(oldValue, newValue);
					displayedCurrencies.Add(render);
					return;
				}
			}
