	...
	public abstract class IClickableMenu
	{
		...
		public virtual void populateClickableComponentList()
		{
			...
			foreach (FieldInfo f in fields)
			{
				...
				if (!(value is ClickableComponent component3))
				{
					if (!(value is List<List<ClickableTextureComponent>> listOfLists))
					{
						if (!(value is InventoryMenu inventoryMenu))
						{
							if (!(value is List<Dictionary<ClickableTextureComponent, CraftingRecipe>> list3))
							{
								if (!(value is Dictionary<int, List<List<ClickableTextureComponent>>> dict2))
								{
								...
								foreach (List<List<ClickableTextureComponent>> value2 in dict2.Values)
								{
									foreach (List<ClickableTextureComponent> list in value2)
									{
										allClickableComponents.AddRange(list);
									}
								}
								continue;
							}
							foreach (Dictionary<ClickableTextureComponent, CraftingRecipe> dict in list3)
							{
								allClickableComponents.AddRange(dict.Keys);
							}
						}
						else
						{
					...
					{
						foreach (ClickableTextureComponent component in item2)
						{
							if (component != null)
							{
