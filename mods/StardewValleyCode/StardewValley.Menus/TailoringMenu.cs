...
namespace StardewValley.Menus
{
	public class TailoringMenu : MenuWithInventory
	{
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (!IsBusy())
			{
				Color color2 = ((base.heldItem == null || (ItemHighlightCache.GetValueOrDefault(base.heldItem)?.LeftSlot ?? false)) ? Color.White : (Color.White * 0.5f));
				if (leftIngredientSpot.item != null)
				{
					blankLeftIngredientSpot.draw(b, color2, 0.87f);
				}
				else
				{
					leftIngredientSpot.draw(b, color2, 0.87f, (int)Game1.currentGameTime.TotalGameTime.TotalMilliseconds % 1000 / 200);
				}
			}
			else
			{
			...
			if (craftResultDisplay.visible)
			{
				...
				if (craftResultDisplay.item != null)
				{
					if (_isMultipleResultCraft)
					{
						...
						question_mark_bounds2.Y -= 8 + (int)questionMarkOffset.Y;
					...
					else
					{
						...
						if (!(item is Hat))
						{
							...
							{
								if (item is Object crafted_object && crafted_object.QualifiedItemId == "(O)71")
								{
			...
			foreach (ClickableComponent c in equipmentIcons)
			{
				...
				case "Hat":
					if (Game1.player.hat.Value != null)
					{
						...
						float transparency = ((!HighlightItems(Game1.player.hat.Value) || Game1.player.hat.Value == base.heldItem || (base.heldItem != null && !(base.heldItem is Hat))) ? 0.5f : 1f);
			...
			if (!IsBusy())
			{
				Color color = ((base.heldItem == null || (ItemHighlightCache.GetValueOrDefault(base.heldItem)?.RightSlot ?? false)) ? Color.White : (Color.White * 0.5f));
				if (rightIngredientSpot.item != null)
				{
					blankRightIngredientSpot.draw(b, color, 0.87f);
				}
				else
				{
					rightIngredientSpot.draw(b, color, 0.87f, (int)Game1.currentGameTime.TotalGameTime.TotalMilliseconds % 1000 / 200);
				}
			}
