	...
	public class MapPage : IClickableMenu
	{
		...
		public virtual void drawMap(SpriteBatch b, bool drawBorders = true, float alpha = 1f)
		{
			...
			if (baseTexture != null)
			{
				...
				b.Draw(baseTexture.Texture, destRect2, baseTexture.SourceRect, Color.White * alpha, 0f, Vector2.Zero, SpriteEffects.None, sortLayer);
				sortLayer += 0.001f;
			}
			MapArea[] array = mapAreas;
			for (int i = 0; i < array.Length; i++)
			{
				...
				foreach (MapAreaTexture overlay in textures)
				{
					...
					b.Draw(overlay.Texture, destRect, overlay.SourceRect, Color.White * alpha, 0f, Vector2.Zero, SpriteEffects.None, sortLayer);
					sortLayer += 0.001f;
				}
			}
			...
			foreach (MapArea area in array)
			{
				if (EnableDebugLines.HasFlag(WorldMapDebugLineType.Tooltips))
				{
					...
					for (int j = 0; j < tooltips.Length; j++)
					{
						...
						Utility.DrawSquare(b, pixelArea3, 2, Color.Blue * alpha);
					}
				}
				...
				foreach (MapAreaPosition worldPosition in area.GetWorldPositions())
				{
					...
					pixelArea = new Rectangle(mapBounds.X + pixelArea.X, mapBounds.Y + pixelArea.Y, pixelArea.Width, pixelArea.Height);
					Utility.DrawSquare(b, pixelArea, 2, Color.Red * alpha);
				}
			}
		}
