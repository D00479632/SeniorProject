...
namespace StardewValley.Menus
{
	public class MasteryTrackerMenu : IClickableMenu
	{
		...
		private static void addCandle(int x, int y, int delay)
		{
			...
			TemporaryAnimatedSprite obj = new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(536, 1945, 8, 8), new Vector2(x, y) * 4f + new Vector2(-3f, -6f) * 4f, flipped: false, 0f, Color.White)
			{
			...
			obj.lightRadius = 1f;
			obj.scale = 3f;
			obj.layerDepth = 0.038500004f;
			obj.delayBeforeAnimationStart = delay;
			obj.startSound = ((delay > 0) ? "fireball" : null);
			obj.drawAboveAlwaysFront = true;
			temporarySprites.Add(obj);
		}
