...
namespace StardewValley.Menus
{
	public class InventoryMenu : IClickableMenu
	{
			...
			: base(xPosition, yPosition, 64 * (((capacity == -1) ? 36 : capacity) / rows), 64 * rows + 16)
		{
			...
			for (int j = 0; j < Game1.player.maxItems.Value; j++)
			{
		...
		public Item tryToAddItem(Item toPlace, string sound = "coin")
		{
			...
			foreach (ClickableComponent item in inventory)
			{
				...
				Item slot2 = ((slotNumber < actualInventory.Count) ? actualInventory[slotNumber] : null);
				if (slot2 == null || !highlightMethod(slot2) || !slot2.canStackWith(toPlace))
				{
					continue;
				}
				int toRemove = toPlace.Stack - slot2.addToStack(toPlace);
			...
			foreach (ClickableComponent item2 in inventory)
			{
				...
				Item slot = ((slotNumber2 < actualInventory.Count) ? actualInventory[slotNumber2] : null);
				if (slotNumber2 >= actualInventory.Count || slot != null)
				{
					continue;
				}
		...
		public override void draw(SpriteBatch b, int red, int green, int blue)
		{
			...
			if (drawSlots)
			{
				for (int l = 0; l < capacity; l++)
				{
					...
					if ((playerInventory || showGrayedOutSlots) && l >= Game1.player.maxItems.Value)
					{
