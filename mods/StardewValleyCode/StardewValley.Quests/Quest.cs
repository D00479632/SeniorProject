			...
			if (questType != null)
			{
				...
				case 8:
				{
					...
					if ((uint)c <= 99u)
					{
						if (c != 'a')
						{
							if (c != 'c' || !(questType == "Location"))
							{
								break;
							}
							if (!TryParseConditions(fields, out var conditions5, out error))
							{
								return LogParseError(id, error);
							}
							if (!ArgUtility.TryGet(conditions5, 0, out var locationName, out error, allowBlank: false, "string locationName"))
							{
					...
					else if (c != 'i')
					{
						...
						if (!ArgUtility.TryGet(conditions3, 0, out var npcName, out error, allowBlank: false, "string npcName") || !ArgUtility.TryGet(conditions3, 1, out var itemId, out error, allowBlank: false, "string itemId") || !ArgUtility.TryGet(conditions3, 2, out var locationOfItem, out error, allowBlank: false, "string locationOfItem") || !ArgUtility.TryGetInt(conditions3, 3, out var tileX, out error, "int tileX") || !ArgUtility.TryGetInt(conditions3, 4, out var tileY, out error, "int tileY"))
						{
							return LogConditionsParseError(id, error);
						}
					...
					else
					{
						...
						if (!ArgUtility.TryGet(conditions2, 0, out var buildingType, out error, allowBlank: false, "string buildingType"))
						{
							return LogConditionsParseError(id, error);
						}
				...
				case 12:
				{
					...
					if (!TryParseConditions(fields, out var conditions7, out error) || !ArgUtility.TryGet(fields, 9, out var targetMessage, out error, allowBlank: false, "string targetMessage"))
					{
						return LogParseError(id, error);
					}
					if (!ArgUtility.TryGet(conditions7, 0, out var npcName2, out error, allowBlank: false, "string npcName") || !ArgUtility.TryGet(conditions7, 1, out var itemId4, out error, allowBlank: false, "string itemId") || !ArgUtility.TryGetOptionalInt(conditions7, 2, out var numberRequired2, out error, 1, "int numberRequired"))
					{
						return LogConditionsParseError(id, error);
					}
					ItemDeliveryQuest itemDeliveryQuest = new ItemDeliveryQuest(npcName2, itemId4);
				...
				case 7:
				{
					...
					if (!ArgUtility.TryGet(conditions, 0, out var monsterName, out error, allowBlank: false, "string monsterName") || !ArgUtility.TryGetInt(conditions, 1, out var numberToKill, out error, "int numberToKill") || !ArgUtility.TryGetOptional(conditions, 2, out var targetNpc, out error, null, allowBlank: true, "string targetNpc") || !ArgUtility.TryGetOptionalBool(conditions, 3, out var ignoreFarmMonsters, out error, defaultValue: true, "bool ignoreFarmMonsters"))
					{
						return LogConditionsParseError(id, error);
					}
				...
				case 11:
				{
					...
					if (!ArgUtility.TryGet(conditions6, 0, out var itemId3, out error, allowBlank: false, "string itemId") || !ArgUtility.TryGetOptionalInt(conditions6, 1, out var numberRequired, out error, 1, "int numberRequired"))
					{
						return LogConditionsParseError(id, error);
					}
				...
				case 14:
					{
						...
						if (!ArgUtility.TryGet(conditions8, 0, out var npcName3, out error, allowBlank: false, "string npcName") || !ArgUtility.TryGet(conditions8, 1, out var itemId5, out error, allowBlank: false, "string itemId") || !ArgUtility.TryGetInt(conditions8, 2, out var friendshipReward, out error, "int friendshipReward") || !ArgUtility.TryGetOptional(conditions8, 3, out var exclusiveQuestId, out error, null, allowBlank: false, "string exclusiveQuestId"))
						{
							return LogConditionsParseError(id, error);
						}
